...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    ...
    internal abstract class CSharpSyntaxNode : GreenNode
    {
        ...
        private static readonly Dictionary<SyntaxNode, Dictionary<CodeAnalysis.SyntaxTrivia, SyntaxNode>> s_structuresTable
            = new Dictionary<SyntaxNode, Dictionary<CodeAnalysis.SyntaxTrivia, SyntaxNode>>();
        ...
        public override SyntaxNode GetStructure(Microsoft.CodeAnalysis.SyntaxTrivia trivia)
        {
            if (trivia.HasStructure)
            {
                ...
                if (parent != null)
                {
                   ...
                   if(!s_structuresTable.ContainsKey(parent))
                    {
                        return CSharp.Syntax.StructuredTriviaSyntax.Create(trivia);
                    }

                    var structsInParent = s_structuresTable[(parent)];
