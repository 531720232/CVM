        ...
        private static SyntaxTree ComputeSyntaxTree(CSharpSyntaxNode node)
        {
            ...
            while (true)
            {
                ...
                if (parent == null)
                {
                    ...
                    CVM.AHelper.CompareExchange(ref node._syntaxTree, CSharpSyntaxTree.CreateWithoutClone(node), null);
                    tree = node._syntaxTree;
                    break;
                }
...
#region SyntaxNode members
        ...
        {
            return SyntaxNodeRemover.RemoveNodes(this, nodes.Cast<CSharpSyntaxNode>().ToArray(), options).AsRootOfNewTreeWithOptionsFrom(this.SyntaxTree);
        }
