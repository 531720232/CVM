...
namespace Microsoft.CodeAnalysis.Syntax.InternalSyntax
{
    ...
    internal class GreenStats
    {
        ...
        {
            CVM.AHelper.Increment(ref stats.greenNodes);
            if (node.IsToken)
            {
                CVM.AHelper.Increment(ref stats.greenTokens);
            }
        }

        internal static void ItemAdded()
        {
            CVM.AHelper.Increment(ref stats.nontermsAdded);
        }
        
        internal static void ItemCacheable()
        {
            CVM.AHelper.Increment(ref stats.cacheableNodes);
        }

        internal static void CacheHit()
        {
            CVM.AHelper.Increment(ref stats.cacheHits);
        }
