...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    {
        private CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>> _lazyDeclaredBases;
        ...
        protected override void CheckInterfaces(DiagnosticBag diagnostics)
        {
            ...
            if (singleDeclaration != null)
            {
                ...
                {
                    var seenInterfaces = new Dictionary<NamedTypeSymbol, NamedTypeSymbol>((IEqualityComparer<NamedTypeSymbol>)EqualsIgnoringTupleNames);
                    foreach (var @interface in interfaces)
                    {
        ...
        internal CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>> GetDeclaredBases(ConsList<Symbol> basesBeingResolved)
        {
        ...
        private CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>> MakeDeclaredBases(ConsList<Symbol> basesBeingResolved, DiagnosticBag diagnostics)
        {
            if (this.TypeKind == TypeKind.Enum)
            {
                // Handled by GetEnumUnderlyingType().
                return new CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>>(null, ImmutableArray<NamedTypeSymbol>.Empty);
            }
            ...
            foreach (var decl in this.declaration.Declarations)
            {
                CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>> one = MakeOneDeclaredBases(newBasesBeingResolved, decl, diagnostics);
            ...
            return new CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>>(baseType, baseInterfacesRO);
        }
        ...
        private CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>> MakeOneDeclaredBases(ConsList<Symbol> newBasesBeingResolved, SingleTypeDeclaration decl, DiagnosticBag diagnostics)
        {
            BaseListSyntax bases = GetBaseListOpt(decl);
            if (bases == null)
            {
            ...
            return new CVM.Tuple2<NamedTypeSymbol, ImmutableArray<NamedTypeSymbol>>(localBase, localInterfaces.ToImmutableAndFree());
        }
