...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal abstract class AssemblySymbol : Symbol, IAssemblySymbolInternal
    {
        ...
        internal NamedTypeSymbol GetTypeByMetadataName(
            ...
            out CVM.Tuple2<AssemblySymbol, AssemblySymbol> conflicts,
        ...
        internal TypeSymbol GetTypeByReflectionType(Type type, bool includeReferences)
        {
            ...
            else if (typeInfo.DeclaringType != null)
            {
                ...
                var currentTypeInfo = typeInfo.IsGenericType ? typeInfo.GetGenericTypeDefinition() : typeInfo;
                var nestedTypes = ArrayBuilder<Type>.GetInstance();
                while (true)
                {
                    ...
                    currentTypeInfo = currentTypeInfo.DeclaringType;
                }

                int i = nestedTypes.Count - 1;
                var symbol = (NamedTypeSymbol)GetTypeByReflectionType(nestedTypes[i], includeReferences);
                if ((object)symbol == null)
                {
                    return null;
                }
                ...
                while (--i >= 0)
                {
                    int forcedArity = nestedTypes[i].GetGenericParameterConstraints().Length - nestedTypes[i + 1].GetGenericParameterConstraints().Length;
            ...
            else
            {
                ...
                MetadataTypeName mdName = MetadataTypeName.FromNamespaceAndTypeName(
                    ...
                    typeInfo.Name,
                    forcedArity: typeInfo.GetGenericArguments().Length);
            ...
            bool ignoreCorLibraryDuplicatedTypes = false)
        {
            ...
            if (assemblyOpt != null)
            {
                ...
                assemblies.AddRange(AotAssemblySymbol.Inst);
            }
            else
            {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                if ((object)result != null)
                {
                    ...
                    if (warnings == null)
                    {
                        conflicts =new CVM.Tuple2<AssemblySymbol, AssemblySymbol> (result.ContainingAssembly, candidate.ContainingAssembly);
                        result = null;
                    }
                    else
                    {
