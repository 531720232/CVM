    ...
    internal sealed class TupleTypeSymbol : WrappedNamedTypeSymbol
    {
            ...
            DiagnosticBag diagnostics = null)
        {
            ...
            if (diagnostics != null && ((SourceModuleSymbol)compilation.SourceModule).AnyReferencedAssembliesAreLinked)
            {
                ...
                CVM_Zone.IsValidEmbeddableType(underlyingType, syntax, diagnostics);
            }
        ...
        /// </summary>
        private static NamedTypeSymbol GetTupleUnderlyingType(ImmutableArray<TypeSymbolWithAnnotations> elementTypes, CSharpSyntaxNode syntax, CVM_Zone compilation, DiagnosticBag diagnostics)
        {
            int numElements = elementTypes.Length;
            int remainder;
        ...
        /// </summary>
        internal static void VerifyTupleTypePresent(int cardinality, CSharpSyntaxNode syntax, CVM_Zone compilation, DiagnosticBag diagnostics)
        {
        ...
        internal static Symbol GetWellKnownMemberInType(NamedTypeSymbol type, WellKnownMember relativeMember)
        {
            ...
            //return CVM_Zone.GetRuntimeMember(type, ref relativeDescriptor, CVM_Zone.SpecialMembersSignatureComparer.Instance,
      ...
      //          Binder.Error(diagnostics, ErrorCode.ERR_PredefinedTypeMemberNotFoundInAssembly, syntax, relativeDescriptor.Name, type, type.ContainingAssembly);
                ...
                foreach (Symbol member in underlyingMembers)
                {
                    switch (member.Kind)
                    {
                        ...
                        case SymbolKind.Field:
                            ...
                            int tupleFieldIndex = currentFieldsForElements.IndexOf(field,(IEqualityComparer<FieldSymbol>) ReferenceEqualityComparer.Instance);
        ...
        {
            var map = new SmallDictionary<Symbol, Symbol>((IEqualityComparer<Symbol>)ReferenceEqualityComparer.Instance);
