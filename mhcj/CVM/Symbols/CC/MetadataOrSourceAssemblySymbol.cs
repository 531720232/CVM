...
//#endif
        ...
        internal override sealed void RegisterDeclaredSpecialType(NamedTypeSymbol corType)
        {
            ...
            if ((object)CVM.AHelper.CompareExchange(ref _lazySpecialTypes[(int)typeId], corType, null) != null)
            {
            ...
            {
                CVM.AHelper.Increment(ref _cachedSpecialTypes);
        ...
        public override ICollection<string> TypeNames
        {
            get
            {
                ...
                {
                    CVM.AHelper.CompareExchange(ref _lazyTypeNames, UnionCollection<string>.Create(this.Modules, m => m.TypeNames), null);
                }
        ...
        public override ICollection<string> NamespaceNames
        {
            get
            {
                ...
                {
                    CVM.AHelper.CompareExchange(ref _lazyNamespaceNames, UnionCollection<string>.Create(this.Modules, m => m.NamespaceNames), null);
                }
...
//#endif
            ...
            if (_lazySpecialTypeMembers == null || ReferenceEquals(_lazySpecialTypeMembers[(int)member], ErrorTypeSymbol.UnknownResultType))
            {
                if (_lazySpecialTypeMembers == null)
                {
                    ...
                    CVM.AHelper.CompareExchange(ref _lazySpecialTypeMembers, specialTypeMembers, null);
                }
                ...
                if (!type.IsErrorType())
                {
                    result = CVM_Zone.GetRuntimeMember(type, ref descriptor, CVM_Zone.SpecialMembersSignatureComparer.Instance, accessWithinOpt: null);
                }

                CVM.AHelper.CompareExchange(ref _lazySpecialTypeMembers[(int)member], result, ErrorTypeSymbol.UnknownResultType);
            }
        ...
        private ConcurrentDictionary<AssemblySymbol, IVTConclusion> AssembliesToWhichInternalAccessHasBeenDetermined
        {
            get
            {
                if (_assembliesToWhichInternalAccessHasBeenAnalyzed == null)
                    CVM.AHelper.CompareExchange(ref _assembliesToWhichInternalAccessHasBeenAnalyzed, new ConcurrentDictionary<AssemblySymbol, IVTConclusion>(), null);
