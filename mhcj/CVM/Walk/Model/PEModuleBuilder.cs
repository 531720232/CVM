...
{
    internal abstract class PEModuleBuilder : PEModuleBuilder<CVM_Zone, SourceModuleSymbol, AssemblySymbol, TypeSymbol, NamedTypeSymbol, MethodSymbol, SyntaxNode, ModuleCompilationState>
    {
                   ...
                   new ModuleCompilationState())
        {
            ...
            _metadataName = specifiedName != CVM_Zone.UnspecifiedModuleAssemblyName ?
                            specifiedName :
                            emitOptions.OutputNameOverride ?? specifiedName;

            AssemblyOrModuleSymbolToModuleRefMap.TryAdd(sourceModule, this);
        ...
        {
            var it1 = SourceModule.ContainingSourceAssembly
        ...
        {
            return SourceModule.GetCustomAttributesToEmit(this).ToArray();
        }
            ...
            ArrayBuilder<Cci.ExportedType> builder)
        {
            if (wellKnownAttributeData?.ForwardedTypes?.Count > 0)
            {
                ...
                var stack = ArrayBuilder<CVM.Tuple2<NamedTypeSymbol , int >>.GetInstance();
                ...
                foreach (NamedTypeSymbol forwardedType in orderedForwardedTypes)
                {
                    ...
                    stack.Push(new CVM.Tuple2<NamedTypeSymbol, int>(originalDefinition, -1));

                    while (stack.Count > 0)
                    {
                        var a = stack.Pop();
                        var type = a.Item1;
                        var parentIndex = a.Item2;
                        ...
                        {
                            stack.Push(new CVM.Tuple2<NamedTypeSymbol, int>(nested[i], index));
                        }
                    }
                }
