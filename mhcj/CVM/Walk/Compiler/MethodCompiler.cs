...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed class MethodCompiler : CSharpSymbolVisitor<TypeCompilationState, object>
    {
        ...
        private Stack<Thread> _compilerTasks;
        ...
        internal MethodCompiler(CVM_Zone compilation, PEModuleBuilder moduleBeingBuiltOpt, bool hasDeclarationErrors,
            ...
            CancellationToken cancellationToken)
        {
          ...
          d,
            ...
            if (compilation.ConcurrentBuild)
            {
                methodCompiler._compilerTasks = new Stack<Thread>();
            }
            ...
            // module or crashing.
            if (moduleBeingBuiltOpt != null && (methodCompiler._globalHasErrors || moduleBeingBuiltOpt.SourceModule.HasBadAttributes) && !diagnostics.HasAnyErrors() && !d)
            {
       ...
       //         WarnUnusedFields(compilation, diagnostics, cancellationToken);
        ...
        //private static MethodSymbol GetEntryPoint(CVM_Zone compilation, PEModuleBuilder moduleBeingBuilt, bool hasDeclarationErrors, DiagnosticBag diagnostics, CancellationToken cancellationToken)
        //{
        ...
        private static void WarnUnusedFields(CVM_Zone compilation, DiagnosticBag diagnostics, CancellationToken cancellationToken)
        {
        ...
        public override object VisitNamespace(NamespaceSymbol symbol, TypeCompilationState arg)
        {
            ...
            if (_compilation.ConcurrentBuild)
            {
                var worker = CompileNamespaceAsTask(symbol);
        ...
        private Thread CompileNamespaceAsTask(NamespaceSymbol symbol)
        {
            var r=new Thread( ()=>{
        ...
        public override object VisitNamedType(NamedTypeSymbol symbol, TypeCompilationState arg)
        {
            ...
            if (_compilation.ConcurrentBuild)
            {
                var worker = CompileNamedTypeAsTask(symbol);
                _compilerTasks.Push(worker);
            }
            else
            {
        ...
        private Thread CompileNamedTypeAsTask(NamedTypeSymbol symbol)
        {
            var r = new Thread(() => {
            ...
            TypeCompilationState compilationState)
        {
            ...
            try
            {
                ...
                //BoundNode Body = default;
                //ExecutableCodeBinder Binder = default;
                CVM.Tuple2<SyntaxNode, BoundNode, ExecutableCodeBinder> forSemanticModel;
            ...
            ImmutableArray<SourceSpan> dynamicAnalysisSpans)
        {
            ...
            var optimizations = compilation.OptimizationLevel;
            ...
            try
            {
                ...
                if (optimizations == 0 && stateMachineTypeOpt != null)
                {
        ...
        private static BoundBlock BindMethodBody(MethodSymbol method, TypeCompilationState compilationState, DiagnosticBag diagnostics, 
                                                 ...
                                                 out CVM.Tuple2<SyntaxNode, BoundNode , ExecutableCodeBinder> forSemanticModel)
        {
            ...
            if ((object)sourceMethod != null)
            {
                ...
                if (bodyBinder != null)
                {
                    ...
                    forSemanticModel = new Tuple2<SyntaxNode, BoundNode, ExecutableCodeBinder>(syntaxNode, methodBody, bodyBinder);
        ...
        internal static BoundExpression BindImplicitConstructorInitializer(
            MethodSymbol constructor, DiagnosticBag diagnostics, CVM_Zone compilation)
        {
