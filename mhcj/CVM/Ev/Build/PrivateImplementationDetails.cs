...
namespace Microsoft.CodeAnalysis.CodeGen
{
    ...
    internal sealed class PrivateImplementationDetails : DefaultTypeDef, Cci.INamespaceTypeDefinition
    {
        ...
        private readonly ConcurrentDictionary<ImmutableArray<byte>, MappedField> _mappedFields =
            new ConcurrentDictionary<ImmutableArray<byte>, MappedField>((IEqualityComparer<ImmutableArray<byte>>)ByteSequenceComparer.Instance);
        ...
        internal void Freeze()
        {
            ...
            fieldsBuilder.AddRange(_mappedFields.Values.ToArray());
            if (_mvidField != null)
            {
                fieldsBuilder.Add(_mvidField);
            }
            fieldsBuilder.AddRange(_instrumentationPayloadRootFields.Values.ToArray());
        ...
        {
            Debug.Assert(IsFrozen);
            return _orderedSynthesizedFields.ToArray();
        }
        ...
        {
            Debug.Assert(IsFrozen);
            return _orderedProxyTypes.OfType<ExplicitSizeStruct>().ToArray();
        }
