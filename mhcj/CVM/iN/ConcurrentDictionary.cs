...
namespace CVM.Collections.Concurrent
{
    /// <summary>
    ...
    public class ConcurrentDictionary<TKey, TValue> : ICollection<KeyValuePair<TKey, TValue>>,ICollection, IDictionary<TKey, TValue>, IDictionary
    {
        /// <summary>
                ...
                lock (tables._locks[lockNo])
                {
                    ...
                    for (Node curr = tables._buckets[bucketNo]; curr != null; curr = curr._next)
                    {
                        ...
                        if (hashcode == curr._hashcode && _comparer.Equals(curr._key, key))
                        {
                            ...
                            if (prev == null)
                            {
                               tables._buckets[bucketNo]= curr._next;
                            }
                            else
                            {
        ...
        private bool TryAddInternal(TKey key, int hashcode, TValue value, bool updateIfExists, bool acquireLock, out TValue resultingValue)
        {
            ...
            while (true)
            {
                ...
                {
                    if (acquireLock)
                        tables._locks[lockNo] =lockTaken;
                   ...
                   tables._buckets[bucketNo]= new Node(key, value, hashcode, tables._buckets[bucketNo]);
                    checked
                    {
                        tables._countPerLock[lockNo]++;
                    }
