...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal abstract class MethodToStateMachineRewriter : MethodToClassRewriter
    {
        ...
        /// </summary>
        private readonly HashSet<Symbol> _hoistedVariables;
        ...
        public MethodToStateMachineRewriter(
            ...
            FieldSymbol state,
            HashSet<Symbol> hoistedVariables,
            IDictionary<Symbol, CapturedSymbolReplacement> nonReusableLocalProxies,
            ...
            : base(slotAllocatorOpt, F.CompilationState, diagnostics)
        {
            ...
            if ((object)thisParameter != null && 
                ...
                F.Compilation.OptimizationLevel == 1)
            {
        ...
        internal HashSet<Symbol> HoistedVariables
        {
            get
            {
        ...
        private bool MightContainReferences(TypeSymbol type)
        {
            ...
            if (!type.IsFromCompilation(this.CompilationState.Compilation)) return true; // perhaps from ref assembly
        ...
        private StateMachineFieldSymbol GetOrAllocateReusableHoistedField(TypeSymbol type, out bool reused, LocalSymbol local = null)
        {
            ...
            Debug.Assert(F.Compilation.OptimizationLevel ==1);
        ...
        private BoundExpression HoistRefInitialization(SynthesizedLocal local, BoundAssignmentOperator node)
        {
            ...
            int syntaxOffset;
            if (F.Compilation.OptimizationLevel ==0)
            {
                    ...
                    if (F.Compilation.OptimizationLevel == 0)
                    {
                        ...
                        if (slotAllocatorOpt == null || 
                            !slotAllocatorOpt.TryGetPreviousHoistedLocalSlotIndex(
                                awaitSyntaxOpt, 
                                F.Compilation.Translate(fieldType, awaitSyntaxOpt, Diagnostics), 
                                kind,
                                id, 
                                Diagnostics,
                                out slotIndex))
