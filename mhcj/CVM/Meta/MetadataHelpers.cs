...
namespace Microsoft.CodeAnalysis
{
    internal static class MetadataHelpers
    {
        ...
        internal static bool IsValidPublicKey(ImmutableArray<byte> key)
        {
        ...
        public static void GetInfoForImmediateNamespaceMembers(
            ...
            out IEnumerable<IGrouping<string, Type>> types,
            out IEnumerable<KeyValuePair<string, IEnumerable<IGrouping<string, Type>>>> namespaces)
        {
            ...
            var nestedTypes = new List<IGrouping<string, Type>>();
            ...
            //   Value – contains a sequence similar to the one passed to this function, but
            //           calculated for the child namespace. 
            var nestedNamespaces = new List<KeyValuePair<string, IEnumerable<IGrouping<string, Type>>>>();
            ...
            using (enumerator)
            {
                if (enumerator.MoveNext())
                {
                    ...
                    List<IGrouping<string, Type>> typesInLastChildNamespace = null;
                        ...
                        else
                        {
                            ...
                            if (typesInLastChildNamespace != null)
                            {
                                ...
                                nestedNamespaces.Add(
                                    new KeyValuePair<string, IEnumerable<IGrouping<string, Type>>>(
                                        lastChildNamespaceName, typesInLastChildNamespace));
                            }

                            typesInLastChildNamespace = new List<IGrouping<string, Type>>();
                ...
                if (names.Count != nestedNamespaces.Count) // nothing to merge otherwise
                {
                    for (int i = 1; i < nestedNamespaces.Count; i++)
                    {
                        ...
                        if (keyIndex != i)
                        {
                            ...
                            nestedNamespaces[keyIndex] = new KeyValuePair<string, IEnumerable<IGrouping<string, Type>>>(primaryPair.Key, primaryPair.Value.Concat(pair.Value));
                            nestedNamespaces[i] = default(KeyValuePair<string, IEnumerable<IGrouping<string, Type>>>);
                        }
                    }
