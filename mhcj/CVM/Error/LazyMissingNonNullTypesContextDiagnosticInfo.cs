...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed class LazyMissingNonNullTypesContextDiagnosticInfo : LazyDiagnosticInfo
    {
        ...
        internal LazyMissingNonNullTypesContextDiagnosticInfo(CVM_Zone compilation, INonNullTypesContext context, TypeSymbolWithAnnotations type)
        {
        ...
        /// </summary>
        public static DiagnosticInfo ReportNullableReferenceTypesIfNeeded(CVM_Zone compilation, INonNullTypesContext context, TypeSymbolWithAnnotations type)
        {
            return !type.IsNull && (type.IsValueType || type.IsErrorType()) ? null : ReportNullableReferenceTypesIfNeeded(compilation, context);
        }

        private static DiagnosticInfo ReportNullableReferenceTypesIfNeeded(CVM_Zone compilation, INonNullTypesContext nonNullTypesContext)
        {
            ...
            if (!compilation.IsFeatureEnabled(featureID))
            {
                ...
                return new CSDiagnosticInfo(availableVersion.GetErrorCode(), featureID.Localize(), (requiredVersion));
            }
