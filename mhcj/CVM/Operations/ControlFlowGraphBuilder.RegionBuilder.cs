...
namespace Microsoft.CodeAnalysis.FlowAnalysis
{
    internal sealed partial class ControlFlowGraphBuilder
    {
        private class RegionBuilder
        {
            ...
            public ArrayBuilder<CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation>> LocalFunctions = null;
            ...
            public void Add(IMethodSymbol symbol, ILocalFunctionOperation operation)
            {
                ...
                {
                    LocalFunctions = ArrayBuilder<CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation>>.GetInstance();
                }

                LocalFunctions.Add(new CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation>(symbol, operation));
            }

            public void AddRange(ArrayBuilder<CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation>> others)
            {
                Debug.Assert(Kind != ControlFlowRegionKind.Root);
                ...
                Debug.Assert(others.All((CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation> tuple) => tuple.Item1.MethodKind == MethodKind.LocalFunction));
            ...
            public ControlFlowRegion ToImmutableRegionAndFree(ArrayBuilder<BasicBlockBuilder> blocks,
                                                              ...
                                                              ImmutableDictionary<IMethodSymbol, CVM.Tuple2<ControlFlowRegion , ILocalFunctionOperation , int >>.Builder localFunctionsMap,
                                                              ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion , int >>.Builder anonymousFunctionsMapOpt,
                                                              ControlFlowRegion enclosing)
            {
                ...
                {
                    foreach (var a in LocalFunctions)
                    {
                        localFunctions.Add(a.Item1);
                    }
                }
                ...
                var result = new ControlFlowRegion(Kind, FirstBlock.Ordinal, LastBlock.Ordinal, subRegions,
                                                   Locals, 
                                                   LocalFunctions?.SelectAsArray((CVM.Tuple2<IMethodSymbol, ILocalFunctionOperation> tuple) => tuple.Item1) ?? default,
                ...
                if (HasLocalFunctions)
                {
                    foreach (var a in LocalFunctions)
                    {
                        localFunctionsMap.Add(a.Item1,new CVM.Tuple2<ControlFlowRegion, ILocalFunctionOperation, int> (result, a.Item2, localFunctionsBefore++));
                    }
                }
                ...
                void setRegion(BasicBlockBuilder block)
                {
                    ...
                    {
                        CVM.Tuple2<ImmutableDictionary<IFlowAnonymousFunctionOperation,  CVM.Tuple2 < ControlFlowRegion , int >>.Builder , ControlFlowRegion > argument = new CVM.Tuple2<ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>.Builder, ControlFlowRegion>(anonymousFunctionsMapOpt, result);
            ...
            private sealed class AnonymousFunctionsMapBuilder : 
                OperationVisitor<CVM.Tuple2<ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>.Builder, ControlFlowRegion>, IOperation>
            {
                ...
                public override IOperation VisitFlowAnonymousFunction(
                    IFlowAnonymousFunctionOperation operation,
                     CVM.Tuple2<ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>.Builder, ControlFlowRegion> argument)
                {
                    argument.Item1.Add(operation, new CVM.Tuple2<ControlFlowRegion, int>(argument.Item2, argument.Item1.Count));
                    return base.VisitFlowAnonymousFunction(operation, argument);
                }

                internal override IOperation VisitNoneOperation(IOperation operation, CVM.Tuple2<ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>.Builder, ControlFlowRegion> argument)
                {
