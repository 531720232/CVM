...
namespace Microsoft.CodeAnalysis.FlowAnalysis
{
    ...
    {
        private readonly CSharp.CVM_Zone            _compilation;
        ...
        private ArrayBuilder<CVM.Tuple2<EvalStackFrame, IOperation >> _evalStack;
        ...
        private ControlFlowGraphBuilder(CSharp.CVM_Zone compilation, CaptureIdDispenser captureIdDispenser)
        {
        ...
        public static ControlFlowGraph Create(IOperation body, ControlFlowGraph parent = null, ControlFlowRegion enclosing = null, CaptureIdDispenser captureIdDispenser = null, in Context context = default)
        {
            ...
            builder._evalStack = ArrayBuilder<CVM.Tuple2<EvalStackFrame, IOperation >>.GetInstance();
            ...
            var localFunctionsMap = ImmutableDictionary.CreateBuilder<IMethodSymbol,CVM.Tuple2<ControlFlowRegion, ILocalFunctionOperation, int>>();
            ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2 < ControlFlowRegion, int>>.Builder anonymousFunctionsMapOpt = null;

            if (builder._haveAnonymousFunction)
            {
                anonymousFunctionsMapOpt = ImmutableDictionary.CreateBuilder<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>();
            }
            ...
            return new ControlFlowGraph(body, parent, builder._captureIdDispenser, ToImmutableBlocks(blocks), region, 
                                        ...
                                        anonymousFunctionsMapOpt?.ToImmutable() ?? ImmutableDictionary<IFlowAnonymousFunctionOperation, CVM.Tuple2<ControlFlowRegion, int>>.Empty);
        }
        ...
        private void VisitStatement(IOperation operation)
        {
            ...
            Debug.Assert(stackDepth == 0 || _evalStack.Peek().Item1 != null);
#endif 
            if (operation == null)
            {
                return;
            ...
            )
        {
            ...
            Debug.Assert(spillingTheStack || _evalStack.All(
                slot => slot.Item2 == null 
                    || slot.Item2.Kind == OperationKind.FlowCaptureReference
                    || slot.Item2.Kind == OperationKind.DeclarationExpression
                    || slot.Item2.Kind == OperationKind.Discard
                    || slot.Item2.Kind == OperationKind.OmittedArgument));
#endif
            if (statement == null)
            {
                return;
        ...
        private void LeaveRegion()
        {
            ...
            foreach (CVM.Tuple2<EvalStackFrame, IOperation > a in _evalStack)
            {
                Debug.Assert((a.Item1 == null) != (a.Item2 == null));

                if (a.Item1 != null)
                {
                    Debug.Assert(enclosed != a.Item1.RegionBuilderOpt);
                }
            }
#endif 
        ...
        private void StartVisitingStatement(IOperation operation)
        {
            ...
            Debug.Assert(_evalStack.Count == 0 || _evalStack.Peek().Item1 != null);
            SpillEvalStack();
        }
        ...
        private EvalStackFrame PushStackFrame()
        {
            ...
            _evalStack.Push(new CVM.Tuple2<EvalStackFrame, IOperation>(frame,  null));
            return frame;
        }
        ...
        private void PopStackFrame(EvalStackFrame frame, bool mergeNestedRegions = true)
        {
            ...
            CVM.Tuple2<EvalStackFrame, IOperation > a= _evalStack.Pop();
            Debug.Assert(frame == a.Item1);
            Debug.Assert(a.Item2 == null);
        ...
        private void SpillEvalStack()
        {
            ...
            {
                var frameOpt  = _evalStack[i].Item1;
                if (frameOpt != null)
                {
                    currentFrameIndex = i;
            ...
            {
                CVM.Tuple2<EvalStackFrame, IOperation > v= _evalStack[i];
                Debug.Assert((v.Item1 == null) != (v.Item2 == null));

                if (v.Item1 != null)
                {
                    currentFrameIndex = i;
                    Debug.Assert(v.Item1.RegionBuilderOpt == null);
                    v.Item1.RegionBuilderOpt = new RegionBuilder(ControlFlowRegionKind.LocalLifetime, isStackSpillRegion: true);
                    EnterRegion(v.Item1.RegionBuilderOpt, spillingStack: true);
                    continue;
                }

                // Declarations cannot have control flow, so we don't need to spill them.
                if (v.Item2.Kind != OperationKind.FlowCaptureReference
                    && v.Item2.Kind != OperationKind.DeclarationExpression
                    && v.Item2.Kind != OperationKind.Discard
                    && v.Item2.Kind != OperationKind.OmittedArgument)
                {
                    ...
                    RegionBuilder currentSpillRegion = _evalStack[currentFrameIndex].Item1.RegionBuilderOpt;
                    ...
                    if (_currentRegion != currentSpillRegion)
                    {
                        ...
                        {
                            IOperation operation = _evalStack[j].Item2;
                            if (operation != null)
                            {
                                if (j < i)
                                {
                    ...
                    AddStatement(new FlowCapture(captureId, v.Item2.Syntax, v.Item2)
#if DEBUG
                                 , spillingTheStack: true
#endif
                                );

                    _evalStack[i] = new CVM.Tuple2<EvalStackFrame, IOperation>( null,  GetCaptureReference(captureId, v.Item2));
        ...
        private void PushOperand(IOperation operation)
        {
            ...
            Debug.Assert(_evalStack.First().Item1 != null);
            Debug.Assert(_evalStack.First().Item2 == null);
            Debug.Assert(_startSpillingAt <= _evalStack.Count);
            Debug.Assert(operation != null);
            _evalStack.Push(new CVM.Tuple2<EvalStackFrame, IOperation>( null, operation));
        }
        ...
        private IOperation PopOperand()
        {
            ...
            CVM.Tuple2<EvalStackFrame, IOperation > v= _evalStack.Pop();
            Debug.Assert(v.Item1 == null);
            Debug.Assert(v.Item2 != null);
        ...
        private IOperation PeekOperand()
        {
            ...
            CVM.Tuple2<EvalStackFrame, IOperation > v= _evalStack.Peek();
            Debug.Assert(v.Item1 == null);
            Debug.Assert(v.Item2 != null);
        ...
        {
           var a = VisitPreservingTupleOperations(operation.LeftOperand, operation.RightOperand);
            return new TupleBinaryOperatorExpression(operation.OperatorKind, a.Item1, a.Item2,
...
oneMoreTime:
        ...
        {
            Debug.Assert(_evalStack.Last().Item1 != null);
        ...
        public override IOperation VisitUsing(IUsingOperation operation, int? captureIdForResult)
        {
            ...
            if (operation.Resources.Kind == OperationKind.VariableDeclarationGroup)
            {
                ...
                var resourceQueue = ArrayBuilder<CVM.Tuple2<IVariableDeclarationOperation, IVariableDeclaratorOperation>>.GetInstance(declarationGroup.Declarations.Length);
                ...
                foreach (IVariableDeclarationOperation declaration in declarationGroup.Declarations)
                {
                    ...
                    {
                        resourceQueue.Add(new CVM.Tuple2<IVariableDeclarationOperation, IVariableDeclaratorOperation>(declaration, declarator));
                    }
                }
            ...
            void processQueue(ArrayBuilder<CVM.Tuple2<IVariableDeclarationOperation, IVariableDeclaratorOperation>> resourceQueueOpt)
            {
                ...
                {
                    var a = resourceQueueOpt.Pop();
                    HandleVariableDeclarator(a.Item1, a.Item2);
                    ILocalSymbol localSymbol = a.Item2.Symbol;
                    processResource(new LocalReferenceExpression(localSymbol, isDeclaration: false, semanticModel: null, a.Item2.Syntax, localSymbol.Type,
                                                                 constantValue: default, isImplicit: true),
                                    resourceQueueOpt);
                }
            }
            ...
            void processResource(IOperation resource, ArrayBuilder<CVM.Tuple2<IVariableDeclarationOperation, IVariableDeclaratorOperation>> resourceQueueOpt)
            {
        ...
        public override IOperation VisitForEachLoop(IForEachLoopOperation operation, int? captureIdForResult)
        {
            ...
            InvocationExpression makeInvocationDroppingInstanceForStaticMethods(IMethodSymbol method, IOperation instance, Lazy1<ImmutableArray<IArgumentOperation>> arguments)
            {
                return makeInvocation(instance.Syntax, method, method.IsStatic ? null : instance, arguments);
            }

            InvocationExpression makeInvocation(SyntaxNode syntax, IMethodSymbol method, IOperation instanceOpt, Lazy1<ImmutableArray<IArgumentOperation>> arguments)
            {
            ...
            ImmutableArray<IArgumentOperation> makeArguments(Lazy1<ImmutableArray<IArgumentOperation>> arguments)
            {
                if (arguments != null)
                {
        ...
        public override IOperation VisitForToLoop(IForToLoopOperation operation, int? captureIdForResult)
        {
           ...
           // (ILocalSymbol loopObject, ForToLoopOperationUserDefinedInfo userDefinedInfo) 
             var a    = ((BaseForToLoopStatement)operation).Info;
            bool isObjectLoop = (a.Item1 != null);
            ImmutableArray<ILocalSymbol> locals = operation.Locals;

            if (isObjectLoop)
            {
                locals = locals.Insert(0, a.Item1);
            }
            ...
            IOperation tryCallObjectForLoopControlHelper(SyntaxNode syntax, WellKnownMember helper)
            {
                ...
                var loopObjectReference = new LocalReferenceExpression(a.Item1, 
                                                                       isDeclaration: isInitialization, 
                                                                       semanticModel: null,
                                                                       operation.LoopControlVariable.Syntax, a.Item1.Type,
                                                                       constantValue: default, isImplicit: true);
            ...
            void initializeLoop()
            {
                ...
                else
                {
                    ...
                    if (a.Item2 != null)
                    {
                        ...
                        IOperation subtraction = Visit(a.Item2.Subtraction.Value);
                        ...
                        VisitAndCapture(a.Item2.GreaterThanOrEqual.Value, positiveFlagId);

                        positiveFlag = GetCaptureReference(positiveFlagId, a.Item2.GreaterThanOrEqual.Value);
            ...
            void checkLoopCondition()
            {
                ...
                else if (a.Item2 != null)
                {
                    ...
                    VisitConditionalBranch(a.Item2.LessThanOrEqual.Value, ref @break, sense: false);
                    LinkBlocks(CurrentBasicBlock, bodyBlock);
                    ...
                    VisitConditionalBranch(a.Item2.GreaterThanOrEqual.Value, ref @break, sense: false);
                    LinkBlocks(CurrentBasicBlock, bodyBlock);
                    _currentBasicBlock = null;
            ...
            void incrementLoopControlVariable()
            {
                ...
                else if (a.Item2 != null)
                {
                    ...
                    IOperation increment = Visit(a.Item2.Addition.Value);
        ...
        public override IOperation VisitInvocation(IInvocationOperation operation, int? captureIdForResult)
        {
            ...
            var a = VisitInstanceWithArguments(instance, operation.Arguments);
            PopStackFrame(frame);
            return new InvocationExpression(operation.TargetMethod, a.Item1, operation.IsVirtual, a.Item2, semanticModel: null, operation.Syntax,
                                            operation.Type, operation.ConstantValue, IsImplicit(operation));
        }

        private CVM.Tuple2<IOperation , ImmutableArray<IArgumentOperation> > VisitInstanceWithArguments(IOperation instance, ImmutableArray<IArgumentOperation> arguments)
        {
            if (instance != null)
            {
            ...
            return new CVM.Tuple2<IOperation, ImmutableArray<IArgumentOperation>>(visitedInstance, visitedArguments);
        }
        ...
        {
            var a = VisitPreservingTupleOperations(operation.Target, operation.Value);
            return new DeconstructionAssignmentExpression(a.Item1, a.Item2, semanticModel: null, operation.Syntax, operation.Type, operation.ConstantValue, IsImplicit(operation));
        }
        ...
        private CVM.Tuple2<IOperation , IOperation > VisitPreservingTupleOperations(IOperation left, IOperation right)
        {
            ...
            PopStackFrame(frame);
            return new CVM.Tuple2<IOperation, IOperation>(visitedLeft, visitedRight);
        }
        ...
        public override IOperation VisitPropertyReference(IPropertyReferenceOperation operation, int? captureIdForResult)
        {
            ...
            var a = VisitInstanceWithArguments(instance, operation.Arguments);
            PopStackFrame(frame);
            return new PropertyReferenceExpression(operation.Property, a.Item1, a.Item2, semanticModel: null,
                                                   operation.Syntax, operation.Type, operation.ConstantValue, IsImplicit(operation));
        }
