...
namespace Microsoft.CodeAnalysis.Operations
{
    ...
    {
        private readonly Lazy1<IOperation> _lazyReference;

        public LazyAddressOfExpression(Lazy1<IOperation> reference, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyArgument;

        public LazyNameOfExpression(Lazy1<IOperation> argument, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyException;

        public LazyThrowExpression(Lazy1<IOperation> exception, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyValue;

        public LazyArgumentOperation(Lazy1<IOperation> value, ArgumentKind argumentKind, IConvertibleConversion inConversionOpt, IConvertibleConversion outConversionOpt, IParameterSymbol parameter, SemanticModel semanticModel, SyntaxNode syntax, bool isImplicit) :
            base(argumentKind, parameter, inConversionOpt, outConversionOpt, semanticModel, syntax, isImplicit)
        {
            _lazyValue = value;
        }
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyDimensionSizes;
        private readonly Lazy1<IArrayInitializerOperation> _lazyInitializer;

        public LazyArrayCreationExpression(Lazy1<ImmutableArray<IOperation>> dimensionSizes, Lazy1<IArrayInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyArrayReference;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyIndices;

        public LazyArrayElementReferenceExpression(Lazy1<IOperation> arrayReference, Lazy1<ImmutableArray<IOperation>> indices, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyElementValues;

        public LazyArrayInitializer(Lazy1<ImmutableArray<IOperation>> elementValues, SemanticModel semanticModel, SyntaxNode syntax, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyTarget;
        private readonly Lazy1<IOperation> _lazyValue;

        public LazySimpleAssignmentExpression(Lazy1<IOperation> target, bool isRef, Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(isRef, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyDeconstructionAssignmentExpression : BaseDeconstructionAssignmentExpression, IDeconstructionAssignmentOperation
    {
        ...
        public LazyDeconstructionAssignmentExpression(Lazy1<IOperation> target, Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyTarget = target ?? throw new System.ArgumentNullException(nameof(target));
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
    ...
    {
        private readonly Lazy1<IOperation> _lazyExpression;

        public LazyDeclarationExpression(Lazy1<IOperation> expression, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyOperation;

        public LazyAwaitExpression(Lazy1<IOperation> operation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyLeftOperand;
        private readonly Lazy1<IOperation> _lazyRightOperand;

        public LazyBinaryOperatorExpression(BinaryOperatorKind operatorKind, Lazy1<IOperation> leftOperand, Lazy1<IOperation> rightOperand, bool isLifted, bool isChecked, bool isCompareText,
                                            IMethodSymbol operatorMethod, IMethodSymbol unaryOperatorMethod, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type,
                                            Optional<object> constantValue, bool isImplicit) :
            base(operatorKind, isLifted, isChecked, isCompareText, operatorMethod, unaryOperatorMethod, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed class LazyTupleBinaryOperatorExpression : BaseTupleBinaryOperatorExpression, ITupleBinaryOperation
    {
        ...
        public LazyTupleBinaryOperatorExpression(BinaryOperatorKind operatorKind, Lazy1<IOperation> leftOperand, Lazy1<IOperation> rightOperand, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit)
            : base(operatorKind, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyLeftOperand = leftOperand ?? throw new System.ArgumentNullException(nameof(leftOperand));
            _lazyRightOperand = rightOperand ?? throw new System.ArgumentNullException(nameof(rightOperand));
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyOperations;

        public LazyBlockStatement(Lazy1<ImmutableArray<IOperation>> operations, ImmutableArray<ILocalSymbol> locals, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyExceptionDeclarationOrExpression;
        private readonly Lazy1<IOperation> _lazyFilter;
        private readonly Lazy1<IBlockOperation> _lazyHandler;

        public LazyCatchClause(Lazy1<IOperation> exceptionDeclarationOrExpression, ITypeSymbol exceptionType, ImmutableArray<ILocalSymbol> locals, Lazy1<IOperation> filter, Lazy1<IBlockOperation> handler, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(exceptionType, locals, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyCompoundAssignmentOperation : BaseCompoundAssignmentExpression
    {
        ...
        public LazyCompoundAssignmentOperation(Lazy1<IOperation> target, Lazy1<IOperation> value, IConvertibleConversion inConversionConvertible, IConvertibleConversion outConversionConvertible, BinaryOperatorKind operatorKind, bool isLifted, bool isChecked, IMethodSymbol operatorMethod, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(inConversionConvertible, outConversionConvertible, operatorKind, isLifted, isChecked, operatorMethod, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyTarget = target;
            _lazyValue = value;
    ...
    {
        private readonly Lazy1<IOperation> _lazyWhenNotNull;
        private readonly Lazy1<IOperation> _lazyOperation;

        public LazyConditionalAccessExpression(Lazy1<IOperation> whenNotNull, Lazy1<IOperation> operation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyCondition;
        private readonly Lazy1<IOperation> _lazyWhenTrue;
        private readonly Lazy1<IOperation> _lazyWhenFalse;

        public LazyConditionalOperation(Lazy1<IOperation> condition, Lazy1<IOperation> whenTrue, Lazy1<IOperation> whenFalse, bool isRef, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(isRef, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyOperand;

        public LazyConversionOperation(Lazy1<IOperation> lazyOperand, IConvertibleConversion convertibleConversion, bool isTryCast, bool isChecked, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(convertibleConversion, isTryCast, isChecked, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyEventReference;
        private readonly Lazy1<IOperation> _lazyHandlerValue;

        public LazyEventAssignmentOperation(Lazy1<IOperation> eventReference, Lazy1<IOperation> handlerValue, bool adds, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(adds, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyInstance;

        public LazyEventReferenceExpression(IEventSymbol @event, Lazy1<IOperation> instance, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(@event, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyExpressionStatement : BaseExpressionStatement, IExpressionStatementOperation
    {
        ...
        public LazyExpressionStatement(Lazy1<IOperation> operation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperation = operation ?? throw new System.ArgumentNullException(nameof(operation));
        }
    ...
    internal sealed partial class LazyVariableInitializer : BaseVariableInitializer, IVariableInitializerOperation
    {
        ...
        public LazyVariableInitializer(Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    internal sealed partial class LazyFieldInitializer : BaseFieldInitializer, IFieldInitializerOperation
    {
        ...
        public LazyFieldInitializer(ImmutableArray<ILocalSymbol> locals, ImmutableArray<IFieldSymbol> initializedFields, Lazy1<IOperation> value, OperationKind kind, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, initializedFields, kind, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    internal sealed partial class LazyFieldReferenceExpression : BaseFieldReferenceExpression, IFieldReferenceOperation
    {
        ...
        public LazyFieldReferenceExpression(IFieldSymbol field, bool isDeclaration, Lazy1<IOperation> instance, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(field, isDeclaration, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInstance = instance ?? throw new System.ArgumentNullException(nameof(instance));
        }
    ...
    {
        private readonly Lazy1<IVariableDeclarationGroupOperation> _lazyVariables;
        private readonly Lazy1<IOperation> _lazyBody;

        public LazyFixedStatement(ImmutableArray<ILocalSymbol> locals, Lazy1<IVariableDeclarationGroupOperation> variables, Lazy1<IOperation> body, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyLoopControlVariable;
        private readonly Lazy1<IOperation> _lazyCollection;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyNextVariables;
        private readonly Lazy1<IOperation> _lazyBody;

        public LazyForEachLoopStatement(ImmutableArray<ILocalSymbol> locals, ILabelSymbol continueLabel, ILabelSymbol exitLabel, Lazy1<IOperation> loopControlVariable, 
                                        Lazy1<IOperation> collection, Lazy1<ImmutableArray<IOperation>> nextVariables, Lazy1<IOperation> body, ForEachLoopOperationInfo info,
                                        SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, continueLabel, exitLabel, info, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyBefore;
        private readonly Lazy1<IOperation> _lazyCondition;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyAtLoopBottom;
        private readonly Lazy1<IOperation> _lazyBody;

        public LazyForLoopStatement(Lazy1<ImmutableArray<IOperation>> before, Lazy1<IOperation> condition, Lazy1<ImmutableArray<IOperation>> atLoopBottom, ImmutableArray<ILocalSymbol> locals, ImmutableArray<ILocalSymbol> conditionLocals,
            ILabelSymbol continueLabel, ILabelSymbol exitLabel, Lazy1<IOperation> body, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, conditionLocals, continueLabel, exitLabel, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal abstract partial class BaseForToLoopStatement : LoopStatement, IForToLoopOperation
    {
        public BaseForToLoopStatement(ImmutableArray<ILocalSymbol> locals, bool isChecked,
                                      CVM.Tuple2<ILocalSymbol, ForToLoopOperationUserDefinedInfo> info,
        ...
        {
            Debug.Assert(info.Item1 == null || info.Item1.Type.SpecialType == SpecialType.System_Object);
        ...
        public CVM.Tuple2<ILocalSymbol, ForToLoopOperationUserDefinedInfo> Info { get; }
    ...
    {
        private readonly Lazy1<IOperation> _lazyLoopControlVariable;
        private readonly Lazy1<IOperation> _lazyInitialValue;
        private readonly Lazy1<IOperation> _lazyLimitValue;
        private readonly Lazy1<IOperation> _lazyStepValue;
        private readonly Lazy1<IOperation> _lazyBody;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyNextVariables;

        public LazyForToLoopStatement(ImmutableArray<ILocalSymbol> locals, bool isChecked,
                                      CVM.Tuple2<ILocalSymbol, ForToLoopOperationUserDefinedInfo> info, 
                                      ILabelSymbol continueLabel, ILabelSymbol exitLabel, Lazy1<IOperation> loopControlVariable, 
                                      Lazy1<IOperation> initialValue, Lazy1<IOperation> limitValue, Lazy1<IOperation> stepValue, Lazy1<IOperation> body, 
                                      Lazy1<ImmutableArray<IOperation>> nextVariables, SemanticModel semanticModel, SyntaxNode syntax, 
                                      ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, isChecked, info, continueLabel, exitLabel, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyLoopControlVariable = loopControlVariable ?? throw new System.ArgumentNullException(nameof(loopControlVariable));
    ...
    internal sealed partial class LazyIncrementExpression : BaseIncrementExpression, IIncrementOrDecrementOperation
    {
        ...
        public LazyIncrementExpression(bool isDecrement, bool isPostfix, bool isLifted, bool isChecked, Lazy1<IOperation> target, IMethodSymbol operatorMethod, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(isDecrement, isPostfix, isLifted, isChecked, operatorMethod, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyTarget = target ?? throw new System.ArgumentNullException(nameof(target));
        }
    ...
    {
        private readonly Lazy1<ImmutableArray<IInterpolatedStringContentOperation>> _lazyParts;

        public LazyInterpolatedStringExpression(Lazy1<ImmutableArray<IInterpolatedStringContentOperation>> parts, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyParts = parts;
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyText;

        public LazyInterpolatedStringText(Lazy1<IOperation> text, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyText = text;
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyExpression;
        private readonly Lazy1<IOperation> _lazyAlignment;
        private readonly Lazy1<IOperation> _lazyFormatString;

        public LazyInterpolation(Lazy1<IOperation> expression, Lazy1<IOperation> alignment, Lazy1<IOperation> formatString, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyChildren;

        public LazyInvalidOperation(Lazy1<ImmutableArray<IOperation>> children, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            // we don't allow null children.
    ...
    {
        private readonly Lazy1<IOperation> _lazyInstance;
        private readonly Lazy1<ImmutableArray<IArgumentOperation>> _lazyArguments;

        public LazyInvocationExpression(IMethodSymbol targetMethod, Lazy1<IOperation> instance, bool isVirtual, Lazy1<ImmutableArray<IArgumentOperation>> arguments, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(targetMethod, isVirtual, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInstance = instance ?? throw new System.ArgumentNullException(nameof(instance));
    ...
    {
        private readonly Lazy1<IEventReferenceOperation> _lazyEventReference;
        private readonly Lazy1<ImmutableArray<IArgumentOperation>> _lazyArguments;

        public LazyRaiseEventStatement(Lazy1<IEventReferenceOperation> eventReference, Lazy1<ImmutableArray<IArgumentOperation>> arguments, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyIsTypeExpression : BaseIsTypeExpression, IIsTypeOperation
    {
        ...
        public LazyIsTypeExpression(Lazy1<IOperation> operand, ITypeSymbol isType, bool isNotTypeExpression, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
    ...
    internal sealed partial class LazyLabeledStatement : BaseLabeledStatement, ILabeledOperation
    {
        ...
        public LazyLabeledStatement(ILabelSymbol label, Lazy1<IOperation> operation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(label, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperation = operation ?? throw new System.ArgumentNullException(nameof(operation));
        }
    ...
    {
        private readonly Lazy1<IBlockOperation> _lazyBody;

        public LazyAnonymousFunctionExpression(IMethodSymbol symbol, Lazy1<IBlockOperation> body, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(symbol, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyBody = body ?? throw new System.ArgumentNullException(nameof(body));
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyTarget;
        public LazyDelegateCreationExpression(Lazy1<IOperation> lazyTarget, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyDynamicMemberReferenceExpression : BaseDynamicMemberReferenceExpression, IDynamicMemberReferenceOperation
    {
        ...
        public LazyDynamicMemberReferenceExpression(Lazy1<IOperation> lazyInstance, string memberName, ImmutableArray<ITypeSymbol> typeArguments, ITypeSymbol containingType, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(memberName, typeArguments, containingType, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyLockedValue;
        private readonly Lazy1<IOperation> _lazyBody;

        public LazyLockStatement(Lazy1<IOperation> lockedValue, Lazy1<IOperation> body, ILocalSymbol lockTakenSymbol, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(lockTakenSymbol, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyMethodReferenceExpression : BaseMethodReferenceExpression, IMethodReferenceOperation
    {
        ...
        public LazyMethodReferenceExpression(IMethodSymbol method, bool isVirtual, Lazy1<IOperation> instance, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(method, isVirtual, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInstance = instance ?? throw new System.ArgumentNullException(nameof(instance));
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyValue;
        private readonly Lazy1<IOperation> _lazyWhenNull;

        public LazyCoalesceExpression(Lazy1<IOperation> value, Lazy1<IOperation> whenNull, IConvertibleConversion convertibleValueConversion, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(convertibleValueConversion, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
    ...
    internal sealed class LazyCoalesceAssignmentOperation : BaseCoalesceAssignmentOperation
    {
        private readonly Lazy1<IOperation> _lazyTarget;
        private readonly Lazy1<IOperation> _lazyWhenNull;
        public LazyCoalesceAssignmentOperation(Lazy1<IOperation> lazyTarget, Lazy1<IOperation> lazyValue, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyTarget = lazyTarget;
    ...
    {
        private readonly Lazy1<IObjectOrCollectionInitializerOperation> _lazyInitializer;
        private readonly Lazy1<ImmutableArray<IArgumentOperation>> _lazyArguments;

        public LazyObjectCreationExpression(IMethodSymbol constructor, Lazy1<IObjectOrCollectionInitializerOperation> initializer, Lazy1<ImmutableArray<IArgumentOperation>> arguments, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(constructor, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyInitializers;

        public LazyAnonymousObjectCreationExpression(Lazy1<ImmutableArray<IOperation>> initializers, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyParameterInitializer : BaseParameterInitializer, IParameterInitializerOperation
    {
        ...
        public LazyParameterInitializer(ImmutableArray<ILocalSymbol> locals, IParameterSymbol parameter, Lazy1<IOperation> value, OperationKind kind, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, parameter, kind, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    internal sealed partial class LazyParenthesizedExpression : BaseParenthesizedExpression, IParenthesizedOperation
    {
        ...
        public LazyParenthesizedExpression(Lazy1<IOperation> operand, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperand = operand ?? throw new System.ArgumentNullException(nameof(operand));
        }
    ...
    internal sealed partial class LazyPropertyInitializer : BasePropertyInitializer, IPropertyInitializerOperation
    {
        ...
        public LazyPropertyInitializer(ImmutableArray<ILocalSymbol> locals, ImmutableArray<IPropertySymbol> initializedProperties, Lazy1<IOperation> value, OperationKind kind, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, initializedProperties, kind, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    internal sealed partial class LazyPropertyReferenceExpression : BasePropertyReferenceExpression, IPropertyReferenceOperation
    {
        ...
        public LazyPropertyReferenceExpression(IPropertySymbol property, Lazy1<IOperation> instance, Lazy1<ImmutableArray<IArgumentOperation>> arguments, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(property, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInstance = instance ?? throw new System.ArgumentNullException(nameof(instance));
    ...
    {
        private readonly Lazy1<IOperation> _lazyMinimumValue;
        private readonly Lazy1<IOperation> _lazyMaximumValue;

        public LazyRangeCaseClause(Lazy1<IOperation> minimumValue, Lazy1<IOperation> maximumValue, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyRelationalCaseClause : BaseRelationalCaseClause, IRelationalCaseClauseOperation
    {
        ...
        public LazyRelationalCaseClause(Lazy1<IOperation> value, BinaryOperatorKind relation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(relation, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyReturnedValue;

        public LazyReturnStatement(OperationKind kind, Lazy1<IOperation> returnedValue, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(kind, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazySingleValueCaseClause : BaseSingleValueCaseClause, ISingleValueCaseClauseOperation
    {
        ...
        public LazySingleValueCaseClause(ILabelSymbol label, Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(label, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    {
        private readonly Lazy1<ImmutableArray<ICaseClauseOperation>> _lazyClauses;
        private readonly Lazy1<IOperation> _lazyCondition;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyBody;

        public LazySwitchCase(ImmutableArray<ILocalSymbol> locals, Lazy1<IOperation> condition, Lazy1<ImmutableArray<ICaseClauseOperation>> clauses, Lazy1<ImmutableArray<IOperation>> body, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyValue;
        private readonly Lazy1<ImmutableArray<ISwitchCaseOperation>> _lazyCases;

        public LazySwitchStatement(ImmutableArray<ILocalSymbol> locals, Lazy1<IOperation> value, Lazy1<ImmutableArray<ISwitchCaseOperation>> cases, ILabelSymbol exitLabel, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, exitLabel, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
            _lazyCases = cases;
    ...
    {
        private readonly Lazy1<IBlockOperation> _lazyBody;
        private readonly Lazy1<ImmutableArray<ICatchClauseOperation>> _lazyCatches;
        private readonly Lazy1<IBlockOperation> _lazyFinallyHandler;

        public LazyTryStatement(Lazy1<IBlockOperation> body, Lazy1<ImmutableArray<ICatchClauseOperation>> catches, Lazy1<IBlockOperation> finallyHandler, ILabelSymbol exitLabel, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(exitLabel, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyBody = body ?? throw new System.ArgumentNullException(nameof(body));
    ...
    {
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyElements;

        public LazyTupleExpression(Lazy1<ImmutableArray<IOperation>> elements, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, ITypeSymbol naturalType, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, naturalType, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IObjectOrCollectionInitializerOperation> _lazyInitializer;
        public LazyTypeParameterObjectCreationExpression(Lazy1<IObjectOrCollectionInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInitializer = initializer ?? throw new System.ArgumentNullException(nameof(initializer));
        }
    ...
    {
        private readonly Lazy1<IObjectOrCollectionInitializerOperation> _lazyInitializer;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyArguments;
        public LazyDynamicObjectCreationExpression(Lazy1<ImmutableArray<IOperation>> arguments, ImmutableArray<string> argumentNames, ImmutableArray<RefKind> argumentRefKinds, Lazy1<IObjectOrCollectionInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(argumentNames, argumentRefKinds, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyArguments = arguments ?? throw new System.ArgumentNullException(nameof(arguments));
            _lazyInitializer = initializer ?? throw new System.ArgumentNullException(nameof(initializer));
    ...
    internal sealed partial class LazyDynamicInvocationExpression : BaseDynamicInvocationExpression, IDynamicInvocationOperation
    {
        private readonly Lazy1<IOperation> _lazyOperation;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyArguments;
        public LazyDynamicInvocationExpression(Lazy1<IOperation> operation, Lazy1<ImmutableArray<IOperation>> arguments, ImmutableArray<string> argumentNames, ImmutableArray<RefKind> argumentRefKinds, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(argumentNames, argumentRefKinds, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperation = operation ?? throw new System.ArgumentNullException(nameof(operation));
            _lazyArguments = arguments ?? throw new System.ArgumentNullException(nameof(arguments));
    ...
    internal sealed partial class LazyDynamicIndexerAccessExpression : BaseDynamicIndexerAccessExpression, IDynamicIndexerAccessOperation
    {
        private readonly Lazy1<IOperation> _lazyOperation;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyArguments;
        public LazyDynamicIndexerAccessExpression(Lazy1<IOperation> operation, Lazy1<ImmutableArray<IOperation>> arguments, ImmutableArray<string> argumentNames, ImmutableArray<RefKind> argumentRefKinds, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(argumentNames, argumentRefKinds, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperation = operation ?? throw new System.ArgumentNullException(nameof(operation));
            _lazyArguments = arguments ?? throw new System.ArgumentNullException(nameof(arguments));
    ...
    internal sealed partial class LazyUnaryOperatorExpression : BaseUnaryOperatorExpression, IUnaryOperation
    {
        ...
        public LazyUnaryOperatorExpression(UnaryOperatorKind unaryOperationKind, Lazy1<IOperation> operand, bool isLifted, bool isChecked, IMethodSymbol operatorMethod, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(unaryOperationKind, isLifted, isChecked, operatorMethod, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperand = operand ?? throw new System.ArgumentNullException(nameof(operand));
        }
    ...
    {
        private readonly Lazy1<IOperation> _lazyResources;
        private readonly Lazy1<IOperation> _lazyBody;

        public LazyUsingStatement(Lazy1<IOperation> resources, Lazy1<IOperation> body, ImmutableArray<ILocalSymbol> locals, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IVariableInitializerOperation> _lazyInitializer;
        private readonly Lazy1<ImmutableArray<IOperation>> _lazyIgnoredArguments;

        public LazyVariableDeclarator(ILocalSymbol symbol, Lazy1<IVariableInitializerOperation> initializer, Lazy1<ImmutableArray<IOperation>> ignoredArguments, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(symbol, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInitializer = initializer ?? throw new System.ArgumentNullException(nameof(initializer));
    ...
    {
        private readonly Lazy1<ImmutableArray<IVariableDeclaratorOperation>> _lazyDeclarators;
        private readonly Lazy1<IVariableInitializerOperation> _lazyInitializer;

        public LazyVariableDeclaration(Lazy1<ImmutableArray<IVariableDeclaratorOperation>> declarators, Lazy1<IVariableInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<ImmutableArray<IVariableDeclarationOperation>> _lazyDeclarations;

        public LazyVariableDeclarationGroupOperation(Lazy1<ImmutableArray<IVariableDeclarationOperation>> declarations, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyWhileLoopStatement : BaseWhileLoopStatement, IWhileLoopOperation
    {
        private readonly Lazy1<IOperation> _lazyCondition;
        private readonly Lazy1<IOperation> _lazyBody;
        private readonly Lazy1<IOperation> _lazyIgnoredCondition;

        public LazyWhileLoopStatement(Lazy1<IOperation> condition, Lazy1<IOperation> body, Lazy1<IOperation> ignoredCondition, ImmutableArray<ILocalSymbol> locals, ILabelSymbol continueLabel, ILabelSymbol exitLabel, bool conditionIsTop, bool conditionIsUntil, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(locals, continueLabel, exitLabel, conditionIsTop, conditionIsUntil, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyCondition = condition ?? throw new System.ArgumentNullException(nameof(condition));
            _lazyBody = body ?? throw new System.ArgumentNullException(nameof(body));
    ...
    internal sealed partial class LazyWithStatement : BaseWithStatement, IWithOperation
    {
        ...
        public LazyWithStatement(Lazy1<IOperation> body, Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyBody = body ?? throw new System.ArgumentNullException(nameof(body));
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
    ...
    {
        private readonly Lazy1<IBlockOperation> _lazyBody;
        private readonly Lazy1<IBlockOperation> _lazyIgnoredBody;

        public LazyLocalFunctionStatement(IMethodSymbol symbol, Lazy1<IBlockOperation> body, Lazy1<IBlockOperation> ignoredBody, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit)
            : base(symbol, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyBody = body ?? throw new System.ArgumentNullException(nameof(body));
    ...
    internal sealed partial class LazyConstantPattern : BaseConstantPattern, IConstantPatternOperation
    {
        ...
        public LazyConstantPattern(Lazy1<IOperation> value, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
        }
    ...
    {
        private readonly Lazy1<IPatternOperation> _lazyPattern;
        private readonly Lazy1<IOperation> _lazyGuardExpression;

        public LazyPatternCaseClause(ILabelSymbol label, Lazy1<IPatternOperation> lazyPattern, Lazy1<IOperation> lazyGuardExpression, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(label, semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyIsPatternExpression : BaseIsPatternExpression, IIsPatternOperation
    {
        ...
        public LazyIsPatternExpression(Lazy1<IOperation> value, Lazy1<IPatternOperation> lazyPattern, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit)
            : base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyValue = value ?? throw new System.ArgumentNullException(nameof(value));
            _lazyPattern = lazyPattern ?? throw new System.ArgumentNullException(nameof(lazyPattern));
    ...
    internal sealed partial class LazyObjectOrCollectionInitializerExpression : BaseObjectOrCollectionInitializerExpression, IObjectOrCollectionInitializerOperation
    {
        ...
        public LazyObjectOrCollectionInitializerExpression(Lazy1<ImmutableArray<IOperation>> initializers, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyInitializedMember;
        private readonly Lazy1<IObjectOrCollectionInitializerOperation> _lazyInitializer;

        public LazyMemberInitializerExpression(Lazy1<IOperation> initializedMember, Lazy1<IObjectOrCollectionInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    internal sealed partial class LazyTranslatedQueryExpression : BaseTranslatedQueryExpression, ITranslatedQueryOperation
    {
        ...
        public LazyTranslatedQueryExpression(Lazy1<IOperation> operation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperation = operation ?? throw new System.ArgumentNullException(nameof(operation));
        }
    ...
    {
        private readonly Lazy1<IBlockOperation> _lazyBlockBody;
        private readonly Lazy1<IBlockOperation> _lazyExpressionBody;

        public LazyMethodBodyOperation(SemanticModel semanticModel, SyntaxNode syntax, Lazy1<IBlockOperation> blockBody, Lazy1<IBlockOperation> expressionBody) :
            base(semanticModel, syntax)
        {
    ...
    {
        private readonly Lazy1<IOperation> _lazyInitializer;
        private readonly Lazy1<IBlockOperation> _lazyBlockBody;
        private readonly Lazy1<IBlockOperation> _lazyExpressionBody;

        public LazyConstructorBodyOperation(ImmutableArray<ILocalSymbol> locals, SemanticModel semanticModel, SyntaxNode syntax, 
                                            Lazy1<IOperation> initializer, Lazy1<IBlockOperation> blockBody, Lazy1<IBlockOperation> expressionBody) :
            base(locals, semanticModel, syntax)
        {
            _lazyInitializer = initializer;
            _lazyBlockBody = blockBody;
    ...
    internal sealed class DiscardOperation : Operation, IDiscardOperation
    {
        ...
        public override IEnumerable<IOperation> Children => new IOperation[0];
    ...
    {
        private readonly Lazy1<IOperation> _lazyGroup;
        private readonly Lazy1<IOperation> _lazyAggregation;

        public LazyAggregateQueryOperation(Lazy1<IOperation> group, Lazy1<IOperation> aggregation, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
    ...
    {
        private readonly Lazy1<IObjectOrCollectionInitializerOperation> _lazyInitializer;
        public LazyNoPiaObjectCreationOperation(Lazy1<IObjectOrCollectionInitializerOperation> initializer, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyInitializer = initializer ?? throw new System.ArgumentNullException(nameof(initializer));
        }
    ...
    {
        private Lazy1<IOperation> _operand;

        public LazyFromEndIndexOperation(bool isLifted, bool isImplicit, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Lazy1<IOperation> operand, IMethodSymbol symbol) :
                    base(isLifted, isImplicit, semanticModel, syntax, type, symbol)
        {
            _operand = new Lazy1<IOperation>(() => Operation.SetParentOperation(operand.Value, this));
        }
    ...
    {
        private Lazy1<IOperation> _leftOperand;
        private Lazy1<IOperation> _rightOperand;

        public LazyRangeOperation(bool isLifted, bool isImplicit, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Lazy1<IOperation> leftOperand, Lazy1<IOperation> rightOperand, IMethodSymbol symbol) :
                    base(isLifted, isImplicit, semanticModel, syntax, type, symbol)
        {
            _leftOperand = new Lazy1<IOperation>(() => Operation.SetParentOperation(leftOperand.Value, this));
            _rightOperand = new Lazy1<IOperation>(() => Operation.SetParentOperation(rightOperand.Value, this));
        }
    ...
    {
        private readonly Lazy1<ImmutableArray<IReDimClauseOperation>> _lazyClauses;

        public LazyReDimOperation(Lazy1<ImmutableArray<IReDimClauseOperation>> clauses, bool preserve, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(preserve, semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyClauses = clauses;
        }
    ...
    internal sealed class LazyReDimClauseOperation : BaseReDimClauseOperation
    {
        ...
        public LazyReDimClauseOperation(Lazy1<IOperation> lazyOperand, Lazy1<ImmutableArray<IOperation>> lazyDimensionSizes, SemanticModel semanticModel, SyntaxNode syntax, ITypeSymbol type, Optional<object> constantValue, bool isImplicit) :
            base(semanticModel, syntax, type, constantValue, isImplicit)
        {
            _lazyOperand = lazyOperand;
