...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class Binder
    {
            ...
            BoundExpression left, BoundExpression right, DiagnosticBag diagnostics)
        {
            ...
            TypeSymbol dynamicType =(TypeSymbol)Compilation.DynamicType;
            ...
            BoundExpression left, BoundExpression right, DiagnosticBag diagnostics)
        {
            ...
            //(ImmutableArray<BoundExpression> leftParts, ImmutableArray<string> leftNames)
             var a   = GetTupleArgumentsOrPlaceholders(left);
          //  (ImmutableArray<BoundExpression> rightParts, ImmutableArray<string> rightNames) =
           var b= GetTupleArgumentsOrPlaceholders(right);
            ReportNamesMismatchesIfAny(left, right, a.Item2, b.Item2, diagnostics);

            int length = a.Item1.Length;
            Debug.Assert(length == b.Item1.Length);

            var operatorsBuilder = ArrayBuilder<TupleBinaryOperatorInfo>.GetInstance(length);

            for (int i = 0; i < length; i++)
            {
                operatorsBuilder.Add(BindTupleBinaryOperatorInfo(node, kind, a.Item1[i], b.Item1[i], diagnostics));
            }
            ...
            TypeSymbol leftTupleType = MakeConvertedType(operators.SelectAsArray(o => o.LeftConvertedTypeOpt), node.Left, a.Item1, a.Item2, isNullable, compilation, diagnostics);
            TypeSymbol rightTupleType = MakeConvertedType(operators.SelectAsArray(o => o.RightConvertedTypeOpt), node.Right, b.Item1, b.Item2, isNullable, compilation, diagnostics);
        ...
        /// - the elements' names
        /// </summary>
        private static CVM.Tuple2<ImmutableArray<BoundExpression> , ImmutableArray<string> > GetTupleArgumentsOrPlaceholders(BoundExpression expr)
        {
            if (expr.Kind == BoundKind.TupleLiteral)
            {
                var tuple = (BoundTupleLiteral)expr;
                return new CVM.Tuple2<ImmutableArray<BoundExpression>, ImmutableArray<string>>(tuple.Arguments, tuple.ArgumentNamesOpt);
            }
            ...
            return new CVM.Tuple2<ImmutableArray<BoundExpression>, ImmutableArray<string>>(placeholders, tupleType.TupleElementNames);
        }
        ...
        private TypeSymbol MakeConvertedType(ImmutableArray<TypeSymbol> convertedTypes, CSharpSyntaxNode syntax,
            ...
            bool isNullable, CVM_Zone compilation, DiagnosticBag diagnostics)
        {
