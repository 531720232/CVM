    ...
    internal sealed class Imports
    {
        ...
        internal string GetDebuggerDisplay()
        {
            return string.Join("; ", 
                ...
                ExternAliases.Select(ea => $"extern alias {ea.Alias.Name}")).ToArray());
        ...
        public static Imports FromGlobalUsings(CVM_Zone compilation)
        {
            ...
            var uniqueUsings = PooledObjects.PooledHashSet<NamespaceOrTypeSymbol>.GetInstance();
        ...
        internal static Imports ExpandPreviousSubmissionImports(Imports previousSubmissionImports, CVM_Zone newSubmission)
        {
        ...
        private static ImmutableArray<AliasAndExternAliasDirective> ConcatExternAliases(ImmutableArray<AliasAndExternAliasDirective> externs1, ImmutableArray<AliasAndExternAliasDirective> externs2)
        {
            ...
            var replacedExternAliases = PooledObjects.PooledHashSet<string>.GetInstance();
        ...
        private static ImmutableArray<AliasAndExternAliasDirective> BuildExternAliases(
            ...
            DiagnosticBag diagnostics)
        {
            CVM_Zone compilation = binder.Compilation;
        ...
        private static void MarkImportDirective(CVM_Zone compilation, CSharpSyntaxNode directive, bool callerIsSemanticModel)
        {
        ...
        private void Validate()
        {
            ...
            foreach (var a in UsingAliases)
            {
                a.Value.Alias.GetAliasTarget(basesBeingResolved: null);
                semanticDiagnostics.AddRange(a.Value.Alias.AliasTargetDiagnostics);
            }

            foreach (var a in UsingAliases)
            {
                a.Value.Alias.CheckConstraints(semanticDiagnostics);
            }
        ...
        {
            foreach (var a in this.UsingAliases)
            {
                AddAliasSymbolToResult(result, a.Value.Alias, options, originalBinder);
            }
