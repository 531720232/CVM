...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        internal CVM_Zone Compilation { get; }
        ...
        /// </summary>
        internal Binder(CVM_Zone compilation)
        {
            Debug.Assert(compilation != null);
            this.Flags = compilation.TopLevelBinderFlags;
        ...
        protected bool CheckOverflowAtRuntime
        {
            get
            {
                ...
                return result == OverflowChecks.Enabled || result == OverflowChecks.Implicit && Compilation.CheckOverflow;
            }
        }
            ...
            CSharpSyntaxNode syntax = null)
        {
            ...
            bool isOptional = false;// WellKnownMembers.IsSynthesizedAttributeOptional(attributeMember);
        ...
        internal string Dump()
        {
            ...
            TreeDumperNode DumpAncestors()
            {
                ...
                for (Binder scope = this; scope != null; scope = scope.Next)
                {
                    ...
                    if (!a.Item1.IsEmpty())
                    {
                        sub.Add(new TreeDumperNode("locals", a.Item1, null));
                    }
                    ...
                    if (a.Item2 != null)
                    {
                        sub.Add(new TreeDumperNode($"scope", $"{a.Item2} ({scope.ScopeDesignator.Kind()})", null));
                    }
                    if (current != null)
                    {
                        sub.Add(current);
                    }
                    current = new TreeDumperNode(a.Item3, null, sub);
                }

                return current;
            }

           CVM.Tuple2<string , string , string > Print(Binder scope)
            {
                var locals = string.Join(", ", scope.Locals.SelectAsArray(s => s.Name).array);
                string snippet = null;
                ...
                return new CVM.Tuple2<string, string, string>(description, snippet, locals);
            }
        }
    }
}
