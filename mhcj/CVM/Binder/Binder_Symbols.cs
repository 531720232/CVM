...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class Binder
    {
                ...
                case SyntaxKind.PointerType:
                    {
                        ...
                        if (Flags.Includes(BinderFlags.GenericConstraintsClause) && elementType.TypeKind == TypeKind.TypeParameter)
                        {
            ...
            NamespaceOrTypeSymbol qualifierOpt)
        {
            ...
            if (CVM.AHelper.IsNullOrWhiteSpace(identifierValueText))
            {
        ...
        private Symbol UnwrapAlias(Symbol symbol, out AliasSymbol alias, DiagnosticBag diagnostics, SyntaxNode syntax, ConsList<Symbol> basesBeingResolved = null)
        {
            ...
            if (symbol.Kind == SymbolKind.Alias)
            {
                ...
                if ((object)type != null)
                {
                    ...
                    var args = new CVM.Tuple2<Binder,DiagnosticBag,SyntaxNode>(this, diagnostics, syntax);
        ...
        internal static NamedTypeSymbol GetSpecialType(CVM_Zone compilation, SpecialType typeId, SyntaxNode node, DiagnosticBag diagnostics)
        {
        ...
        internal static bool TryGetSpecialTypeMember<TSymbol>(CVM_Zone compilation, SpecialMember specialMember, SyntaxNode syntax, DiagnosticBag diagnostics, out TSymbol symbol)
        ...
        internal static Symbol GetWellKnownTypeMember(CVM_Zone compilation, WellKnownMember member, DiagnosticBag diagnostics, Location location = null, SyntaxNode syntax = null, bool isOptional = false)
        {
        ...
        internal static Symbol GetWellKnownTypeMember(CVM_Zone compilation, WellKnownMember member, out DiagnosticInfo diagnosticInfo, bool isOptional = false)
        {
                ...
                else
                {
                    ...
                    else
                    {
                            ...
                            ((SourceModuleSymbol)this.Compilation.SourceModule).AnyReferencedAssembliesAreLinked)
                        {
                           ...
                           CVM_Zone.IsValidEmbeddableType((NamedTypeSymbol)singleResult, where, diagnostics);
                        }
        ...
        private static BestSymbolLocation GetLocation(CVM_Zone compilation, Symbol symbol)
        {
        ...
        return    Compilation.SourceAssembly.CorLibrary;
        ...
        private static CSDiagnosticInfo GetFeatureAvailabilityDiagnosticInfo(SyntaxTree tree, MessageID feature)
        {
            ...
            {
                return new CSDiagnosticInfo(availableVersion.GetErrorCode(), feature.Localize(), (requiredVersion));
            }
