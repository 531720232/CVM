    ...
    internal partial class Binder
    {
            ...
            out BoundExpression valueBeforeConversion)
        {
            ...
            return new BoundParameterEqualsValue(defaultValueSyntax, parameter, defaultValueBinder.GetDeclaredLocalsForScope(defaultValueSyntax),
                              defaultValueBinder.GenerateConversionForAssignment(parameter.Type.TypeSymbol, valueBeforeConversion, diagnostics, isDefaultParameter: true));
        }
        ...
        private BoundExpression BindTupleExpression(TupleExpressionSyntax node, DiagnosticBag diagnostics)
        {
            ...
            // prepare names
           var a = ExtractTupleElementNames(arguments, diagnostics);
            ...
            if (hasNaturalType)
            {
                ...
                tupleTypeOpt = TupleTypeSymbol.Create(node.Location, elements, locations, a.Item1,
                    this.Compilation, syntax: node, diagnostics: diagnostics, shouldCheckConstraints: true,
                    errorPositions: disallowInferredNames ?a.Item2  : default(ImmutableArray<bool>));
            }
            else
            {
            ...
            return new BoundTupleLiteral(node, a.Item1, a.Item2, boundArguments.ToImmutableAndFree(), tupleTypeOpt); //..hasErrors);
        }

        private static CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool> , bool > ExtractTupleElementNames(
            SeparatedSyntaxList<ArgumentSyntax> arguments, DiagnosticBag diagnostics)
        {
            ...
            return new CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>, bool>(result.Item1, result.Item2, hasErrors);
        }

        private static CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>> MergeTupleElementNames(
            ArrayBuilder<string> elementNames, ArrayBuilder<string> inferredElementNames)
        {
            if (elementNames == null)
            {
                ...
                {
                    return new CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>>(default(ImmutableArray<string>), default(ImmutableArray<bool>));
                }
                else
                {
                    var finalNames = inferredElementNames.ToImmutable();
                    return new CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>>(finalNames, finalNames.SelectAsArray(n => n != null));
                }
            }

            if (inferredElementNames == null)
            {
                return new CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>>(elementNames.ToImmutable(), default(ImmutableArray<bool>));
            }
            ...
            return new CVM.Tuple2<ImmutableArray<string>, ImmutableArray<bool>>(elementNames.ToImmutable(), builder.ToImmutableAndFree());
        }
                ...
                case SymbolKind.Parameter:
                    {
                        ...
                        if (IsBadLocalOrParameterCapture(parameter, parameter.Type.TypeSymbol, parameter.RefKind))
                        {
                            isError = true;
        ...
        private BoundExpression BindCast(CastExpressionSyntax node, DiagnosticBag diagnostics)
        {
            ...
            TypeSymbolWithAnnotations targetTypeWithNullability = this.BindType(node.Type, diagnostics);
            ...
            else if (hasNames)
            {
                ...
                if (!hadLangVersionError && !Compilation.LanguageVersion.AllowNonTrailingNamedArguments())
                {
                    ...
                    Error(diagnostics, ErrorCode.ERR_NamedArgumentSpecificationBeforeFixedArgument, argumentSyntax,
                        (MessageID.IDS_FeatureNonTrailingNamedArguments.RequiredVersion()));
            ...
            StackAllocArrayCreationExpressionSyntax node, DiagnosticBag diagnostics)
        {
            ...
            return node.Initializer == null
                ? new BoundStackAllocArrayCreation(node, elementType.TypeSymbol, count, initializerOpt: null, type, hasErrors: hasErrors)
                : BindStackAllocWithInitializer(node, node.Initializer, type, elementType.TypeSymbol, count, diagnostics, hasErrors);
        }
        ...
        private TypeSymbol GetStackAllocType(SyntaxNode node, TypeSymbolWithAnnotations elementType, DiagnosticBag diagnostics, out bool hasErrors)
        {
            ...
            ImmutableArray<BoundExpression> boundInitExprOpt = default)
        {
            ...
            boundInitExprOpt = boundInitExprOpt.SelectAsArray((expr, t) => GenerateConversionForAssignment(t.Item1, expr, t.Item2),new CVM.Tuple2<TypeSymbol, DiagnosticBag>(elementType, diagnostics));
