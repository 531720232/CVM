...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed partial class BoundLambda : IBoundLambdaOrFunction
    {
        ...
        public TypeSymbolWithAnnotations GetInferredReturnType(ConversionsBase conversions, NullableWalker.VariableState nullableState, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            {
                var returnTypes = ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>>.GetInstance();
        ...
        /// </summary>
        internal static InferredLambdaReturnType InferReturnType(ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> returnTypes, CVM_Zone compilation, ConversionsBase conversions, TypeSymbol delegateType, bool isAsync)
        {
            var types = ArrayBuilder<TypeSymbolWithAnnotations>.GetInstance();
            bool hasReturnWithoutArgument = false;
            RefKind refKind = RefKind.None;
            foreach (var a in returnTypes)
            {
                if (a.Item1 != RefKind.None)
                {
                    refKind = a.Item1;
                }
                if ((object)a.Item2.TypeSymbol == NoReturnExpression)
                {
        ...
        {
            private readonly ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> _builder;

            private BlockReturns(ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> builder)
            {
                _builder = builder;
            }

            public static void GetReturnTypes(ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> builder, BoundBlock block)
            {
            ...
            public override BoundNode VisitReturnStatement(BoundReturnStatement node)
            {
                ...
                _builder.Add(new CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>(node.RefKind, TypeSymbolWithAnnotations.Create(type)));
                return null;
            }
        }
    }
    ...
    internal abstract class UnboundLambdaState
    {
        ...
        private ImmutableDictionary<NamedTypeSymbol, BoundLambda> _bindingCache = ImmutableDictionary<NamedTypeSymbol, BoundLambda>.Empty.WithComparers((IEqualityComparer<NamedTypeSymbol>)TypeSymbol.EqualsConsiderEverything);
