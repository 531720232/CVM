...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed partial class NullableWalker : DataFlowPassBase<NullableWalker.LocalState>
    {
        ...
        /// </summary>
        private readonly ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> _returnTypes;
        ...
        private NullableWalker(
            ...
            BoundNode node,
            ArrayBuilder<CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>> returnTypes,
            ...
            : base(compilation, method, node, new EmptyStructTypeCache(compilation, dev12CompilerCompatibility: false), trackUnassignments: true)
        {
            ...
            if (initialState != null)
            {
                ...
                foreach (var a in initialState.VariableSlot)
                {
                    Debug.Assert(a.Value < nextVariableSlot);
                    _variableSlot.Add(a.Key, a.Value);
                }
        ...
        private void Populate(ref LocalState state, int start)
        {
            ...
            {
                var a = GetDefaultState(ref state, slot);
                state[slot] = a.Item1;
                state.SetAssigned(slot, a.Item2);
            }
        }

        private CVM.Tuple2<NullableAnnotation,  bool> GetDefaultState(ref LocalState state, int slot)
        {
            if (slot == 0)
            {
                return new CVM.Tuple2<NullableAnnotation, bool> (NullableAnnotation.Unknown, false);
            }
                ...
                case SymbolKind.Parameter:
                    {
                        ...
                        return new CVM.Tuple2<NullableAnnotation, bool>(parameterType.NullableAnnotation, true);
                    }
                ...
                case SymbolKind.Event:
                    {
                        ...
                        return new CVM.Tuple2<NullableAnnotation, bool>(symbol.GetTypeOrReturnType().NullableAnnotation, true);
                    }
                default:
...
#region Visitors
        ...
        private void VisitPattern(BoundExpression expression, TypeSymbolWithAnnotations expressionResultType, BoundPattern pattern)
        {
            ...
            switch (pattern.Kind)
            {
                case BoundKind.ConstantPattern:
                    ...
                    {
                        case true:
                            whenTrue = NullableAnnotation.NullableBasedOnAnalysis;
                            whenFalse = NullableAnnotation.NotNullableBasedOnAnalysis;
                            break;
                        case false:
                            whenTrue = NullableAnnotation.NotNullableBasedOnAnalysis;
                ...
                case BoundKind.DeclarationPattern:
                    ...
                    {
                        whenTrue = NullableAnnotation.NotNullableBasedOnAnalysis; // the pattern tells us the expression is not null
                        whenFalse = expressionResultType.NullableAnnotation;
                    }
                    break;
            }
        ...
        protected override BoundNode VisitReturnStatementNoAdjust(BoundReturnStatement node)
        {
            ...
            if (_returnTypes != null)
            {
                ...
                _returnTypes.Add(new CVM.Tuple2<RefKind, TypeSymbolWithAnnotations>(node.RefKind, result));
                return null;
            }
        ...
        private ArrayTypeSymbol VisitArrayInitializer(BoundArrayCreation node)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                var a = RemoveConversion(elementBuilder[i], includeExplicitConversions: false);
                elementBuilder[i] = a.Item1;
                conversionBuilder.Add(a.Item2);
                resultBuilder.Add(VisitRvalueWithResult(a.Item1));
            }
                ...
                if (op == BinaryOperatorKind.Equal || op == BinaryOperatorKind.NotEqual)
                {
                    ...
                    if (operandComparedToNull != null)
                    {
                        ...
                        if (slotBuilder.Count != 0)
                        {
                            ...
                            {
                                state[slot] = NullableAnnotation.NotNullableBasedOnAnalysis;
                            }
                        }
        ...
        public override BoundNode VisitConditionalOperator(BoundConditionalOperator node)
        {
            ...
            BoundExpression consequence=default;
            BoundExpression alternative=default;
            Conversion consequenceConversion=default;
            Conversion alternativeConversion=default;
            TypeSymbolWithAnnotations consequenceResult=default;
            TypeSymbolWithAnnotations alternativeResult=default;
            ...
            if (isConstantTrue)
            {
              var a= visitConditionalOperand(alternativeState, node.Alternative);
                 alternative = a.Item1;
                alternativeConversion = a.Item2;
                alternativeResult = a.Item3;

                var b = visitConditionalOperand(consequenceState, node.Consequence);
                consequence = b.Item1;
                consequenceConversion = b.Item2;
                consequenceResult = b.Item3;
            ...
            else if (isConstantFalse)
            {
                ...
                var a = visitConditionalOperand(consequenceState, node.Consequence); 
                var b= visitConditionalOperand(alternativeState, node.Alternative);
                alternative = b.Item1;
                alternativeResult = b.Item3;
                //    (consequence, consequenceConversion, consequenceResult) = visitConditionalOperand(consequenceState, node.Consequence);
                //  (alternative, alternativeConversion, alternativeResult) = visitConditionalOperand(alternativeState, node.Alternative);
            ...
            else
            {
                ...
                var a= visitConditionalOperand(consequenceState, node.Consequence);
                consequence = a.Item1;
                consequenceConversion = a.Item2;
                consequenceResult = a.Item3;
                //   (consequence, consequenceConversion, consequenceResult) = visitConditionalOperand(consequenceState, node.Consequence);
                Unsplit();
                var b = visitConditionalOperand(alternativeState, node.Alternative);
                alternative = b.Item1;
                alternativeConversion = b.Item2;
                alternativeResult = b.Item3;
                // (alternative, alternativeConversion, alternativeResult) = visitConditionalOperand(alternativeState, node.Alternative);
                Unsplit();
            ...
            CVM.Tuple2<BoundExpression, Conversion, TypeSymbolWithAnnotations> visitConditionalOperand(LocalState state, BoundExpression operand)
            {
                ...
                {
                   var a = RemoveConversion(operand, includeExplicitConversions: false);
                    operand = a.Item1;
                    conversion = a.Item2;
                    Visit(operand);
                }
                return new CVM.Tuple2<BoundExpression, Conversion, TypeSymbolWithAnnotations>(operand, conversion, _resultType);
            }
        }
        ...
        public override BoundNode VisitCall(BoundCall node)
        {
            ...
            var a = RemoveArgumentConversions(node.Arguments, refKindsOpt);
            ...
            method = VisitArguments(node, a.Item1, refKindsOpt, method.Parameters, node.ArgsToParamsOpt,
                node.Expanded, node.InvokedAsExtensionMethod, a.Item2, method);
            ...
            bool expanded, ImmutableArray<ParameterSymbol> parameters, ImmutableArray<int> argsToParamsOpt)
        {
            ...
            {
               var a = GetCorrespondingParameter(i, parameters, argsToParamsOpt, expanded);
                FlowAnalysisAnnotations annotations = a.Item1?.FlowAnalysisAnnotations ?? FlowAnalysisAnnotations.None;

                annotations = removeInapplicableAnnotations(a.Item1, annotations);
            ...
            bool expanded)
        {
          ...
          var a = RemoveArgumentConversions(arguments, refKindsOpt);
            arguments = a.Item1;
            conversions = a.Item2;
            ...
            ImmutableArray<FlowAnalysisAnnotations> annotations)
        {
            ...
            for (int i = 0; i < arguments.Length; i++)
            {
                ...
                if (notNullWhenTrue)
                {
                    this.StateWhenTrue[slot] = NullableAnnotation.NotNullableBasedOnAnalysis;
                }
                if (notNullWhenFalse)
                {
                    this.StateWhenFalse[slot] = NullableAnnotation.NotNullableBasedOnAnalysis;
        ...
        private static CVM.Tuple2<ImmutableArray<BoundExpression> , ImmutableArray<Conversion>> RemoveArgumentConversions(
            ImmutableArray<BoundExpression> arguments,
            ImmutableArray<RefKind> refKindsOpt)
        {
            int n = arguments.Length;
            ...
            if (n > 0)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    {
                        var before = argument;
                        var a= RemoveConversion(argument, includeExplicitConversions: false);
                        argument = a.Item1;
                        conversion = a.Item2;
                        if (argument != before)
                        {
            ...
            return new CVM.Tuple2<ImmutableArray<BoundExpression>, ImmutableArray<Conversion>>(arguments, conversions);
        }
        ...
        private static CVM.Tuple2<ParameterSymbol , TypeSymbolWithAnnotations> GetCorrespondingParameter(
            int argumentOrdinal,
            ImmutableArray<ParameterSymbol> parameters,
            ImmutableArray<int> argsToParamsOpt,
            bool expanded)
        {
            ...
            {
                return new CVM.Tuple2<ParameterSymbol, TypeSymbolWithAnnotations>(default, default);
            }
            ...
            return new CVM.Tuple2<ParameterSymbol, TypeSymbolWithAnnotations>(parameter, type);
        }
        ...
        /// </summary>
        private static CVM.Tuple2<BoundExpression  ,Conversion> RemoveConversion(BoundExpression expr, bool includeExplicitConversions)
        {
            ...
            while (true)
            {
                ...
                {
                    return new CVM.Tuple2<BoundExpression, Conversion>(expr, Conversion.Identity);
                }
                ...
                if (group == null)
                {
                    ...
                    return new CVM.Tuple2<BoundExpression, Conversion>(expr, conversion.Conversion);
                }
            }
            return new CVM.Tuple2<BoundExpression, Conversion>(expr, group?.Conversion ?? Conversion.Identity);
        }
        ...
        public override BoundNode VisitConversion(BoundConversion node)
        {
            ...
            CVM.Tuple2 < BoundExpression, Conversion > c= RemoveConversion(node, includeExplicitConversions: true);
            TypeSymbolWithAnnotations operandType = VisitRvalueWithResult(c.Item1);
        ...
        private TypeSymbolWithAnnotations VisitOptionalImplicitConversion(BoundExpression expr, TypeSymbolWithAnnotations targetTypeOpt, bool useLegacyWarnings, AssignmentKind assignmentKind)
        {
            ...
            var a = RemoveConversion(expr, includeExplicitConversions: false);
            var operandType = VisitRvalueWithResult(a.Item1);
            ...
            switch (conversion.Kind)
            {
                ...
                case ConversionKind.Boxing:
                    if (!operandType.IsNull && operandType.IsValueType)
                    {
                        ...
                        resultAnnotation = operandType.IsNullableType() ? NullableAnnotation.NullableBasedOnAnalysis : NullableAnnotation.NotNullableBasedOnAnalysis;
                    }
        ...
        public override BoundNode VisitLiteral(BoundLiteral node)
        {
            ...
            //if (this.State.Reachable) // Consider reachability: see https://github.com/dotnet/roslyn/issues/28798
            {
                ...
                {
                    _resultType = TypeSymbolWithAnnotations.Create(node.Type, constant.IsNull ? NullableAnnotation.NullableBasedOnAnalysis : NullableAnnotation.NotNullableBasedOnAnalysis);
                }
                else
                {
                    SetResult(node);
        ...
        protected override bool IntersectWith(ref LocalState self, ref LocalState other)
        {
            if (self.Reachable == other.Reachable)
            {
                ...
                for (int slot = 1; slot < self.Capacity; slot++)
                {
                    NullableAnnotation selfAnnotation = self[slot];
                    NullableAnnotation otherAnnotation = other[slot];
                    NullableAnnotation intersection = selfAnnotation.JoinForFlowAnalysisBranches(otherAnnotation,new CVM.Tuple2<int,NullableWalker>(slot, this), isPossiblyNullableReferenceTypeTypeParameterDelegate);
        ...
        private readonly static Func<CVM.Tuple2<int , NullableWalker>, bool> isPossiblyNullableReferenceTypeTypeParameterDelegate = args =>
        {
            Symbol symbol = args.Item2.variableBySlot[args.Item1].Symbol;
