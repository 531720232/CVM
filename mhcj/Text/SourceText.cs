...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public abstract class SourceText
    {
        ...
        private static string Decode(Stream stream, Encoding encoding, out Encoding actualEncoding)
        {
            ...
            using (var reader = new StreamReader(stream, encoding, true,  Math.Min(4096, length)))
            {
                string text = reader.ReadToEnd();
        ...
        internal ImmutableArray<byte> PrecomputedEmbeddedTextBlob
        {
            get
            {
                return _precomputedEmbeddedTextBlob;
            }
        }
        /// <summary>
        ...
        public virtual SourceTextContainer Container
        {
            get
            {
                ...
                {
                    _lazyContainer = new StaticContainer(this);
                   // CVM.AHelper.CompareExchange(ref _lazyContainer, new StaticContainer(this), null);
                }
        ...
        public ImmutableArray<byte> GetChecksum()
        {
            if (_lazyChecksum.IsDefault)
            {
                ...
                {
                 //   CVM.AHelper.Initialize(ref _lazyChecksum, CalculateChecksum(stream, _checksumAlgorithm));
                }
            }
        ...
        public virtual SourceText WithChanges(IEnumerable<TextChange> changes)
        {
            ...
            foreach (var change in changes)
            {
                ...
                {
                    var len = change.NewText != null ? change.NewText.Length : -1;
                ...
                var int_n = change.NewText != null ? change.NewText.Length : 0;

                var newTextLength = int_n;
        ...
        /// </summary>
        public virtual IList<TextChangeRange> GetChangeRanges(SourceText oldText)
        {
            if (oldText == null)
            {
                throw new ArgumentNullException("");
        ...
        public TextLineCollection Lines
        {
            get
            {
                ...
                return info ?? CVM.AHelper.CompareExchange(ref _lazyLineInfo, info = GetLinesCore(), null) ?? info;
            }
        }
