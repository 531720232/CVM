...
namespace Roslyn.Utilities
{
    ...
    internal class StringTable
    {
        ...
        internal string Add(char[] chars, int start, int len)
        {
            ...
            Array.Copy(chars, start, span, 0, span.Length);
        ...
        private static string FindSharedEntry(char[] chars, int start, int len, int hashCode)
        {
            ...
            for (int i = 1; i < SharedBucketSize + 1; i++)
            {
                ...
                if (e != null)
                {
                    var c_1 = new char[len];
                    Array.Copy(chars, start, c_1, 0, c_1.Length);
                    if (hash == hashCode && TextEquals(e,c_1))
                    {
                        break;
                    }
        ...
        private static string FindSharedEntryASCII(int hashCode, byte[] asciiChars)
        {
            var arr = s_sharedTable;
            int idx = SharedIdxFromHash(hashCode);
        ...
        private static string AddSharedSlow(int hashCode, byte[] utf8Bytes, bool isAscii)
        {
            string text;

            
             
                    text = Encoding.UTF8.GetString(utf8Bytes,0,utf8Bytes.Length);
        ...
        {
            return CVM.AHelper.Increment(ref StringTable.s_sharedRandom);
        }
        ...
        internal static bool TextEqualsASCII(string text, byte[] ascii)
        {
            ...
            {
                Debug.Assert((ascii[i] & 0x80) == 0, "The  input to this method must be valid ASCII.");
            }
#endif
        ...
        { return text == array.ToCharArray(); }// StringComparison.Ordinal);
    }
}
