...
namespace CVM.Collections.Immutable
{
    public sealed partial class ImmutableList<T>
    {
        ...
        internal sealed class Node : IBinaryTree<T>, IEnumerable<T>
        {
            ...
            /// Gets the left branch of this node.
            /// </summary>
            IBinaryTree<T> IBinaryTree<T>.Left { get { return _left; } }

            /// <summary>
            /// Gets the right branch of this node.
            /// </summary>
            IBinaryTree<T> IBinaryTree<T>.Right { get { return _right; } }
            ...
            /// Gets the key.
            /// </summary>
            internal T Key { get { return _key; } }
            ...
            /// A <see cref="IEnumerator{T}"/> that can be used to iterate through the collection.
            /// </returns>
     // internal, never called, but here for interface implementation
     ...
     // internal, never called, but here for interface implementation
            ...
            /// A <see cref="IEnumerator{T}"/> that can be used to iterate through the collection.
            /// </returns>
            internal Enumerator GetEnumerator(Builder builder) { return new Enumerator(this, builder);
            }
            /// <summary>
            ...
            internal Node Reverse() { return this.Reverse(0, this.Count);
            }
            /// <summary>
            ...
            /// </summary>
            internal Node Sort() { return this.Sort(Comparer<T>.Default);
            }
            /// <summary>
            /// Sorts the elements in the entire <see cref="ImmutableList{T}"/> using
            ...
            /// </param>
            /// <returns>The sorted list.</returns>
            internal Node Sort(IComparer<T> comparer) { return this.Sort(0, this.Count, comparer);
            }
            /// <summary>
            ...
            internal int IndexOf(T item, IEqualityComparer<T> equalityComparer) { return this.IndexOf(item, 0, this.Count, equalityComparer);
            }
            /// <summary>
            /// Searches for the specified object and returns the zero-based index of the
            /// first occurrence within the range of elements in the <see cref="ImmutableList{T}"/>
            ...
            internal int IndexOf(T item, int index, int count, IEqualityComparer<T> equalityComparer)
            {
                Requires.Range(index >= 0, "");
                Requires.Range(count >= 0, "");
                Requires.Range(count <= this.Count, "");
                Requires.Range(index + count <= this.Count, "");
            ...
            {
                Requires.Range(index >= 0, "");
                Requires.Range(count >= 0 && count <= this.Count, "");
            ...
            internal void CopyTo(T[] array)
            {
                ...
                Requires.Range(array.Length >= this.Count, "");
            ...
            {
                Requires.NotNull(array, "");
                Requires.Range(arrayIndex >= 0, "");
                Requires.Range(array.Length >= arrayIndex + this.Count, "");
            ...
            internal void CopyTo(int index, T[] array, int arrayIndex, int count)
            {
                ...
                Requires.Range(index + count <= this.Count, "");
                Requires.Range(arrayIndex >= 0, "");
                Requires.Range(arrayIndex + count <= array.Length, "");
            ...
            internal ImmutableList<TOutput>.Node ConvertAll<TOutput>(Func<T, TOutput> converter)
            {
                ...
                return root.AddRange(System.Linq.Enumerable.Select(this, converter));
            }
            ...
            {
                Requires.NotNull(match, "");
                Requires.Range(startIndex >= 0, "");
                Requires.Range(count >= 0, "");
                Requires.Range(startIndex + count <= this.Count, "");
            ...
            internal int FindLastIndex(int startIndex, int count, Predicate<T> match)
            {
                ...
                Requires.Range(startIndex >= 0, "");
                Requires.Range(count <= this.Count, "");
                Requires.Range(startIndex - count + 1 >= 0, "");
            ...
            /// </returns>
            private bool IsBalanced { get { return unchecked((uint)(this.BalanceFactor + 1)) <= 2;
                } }
            /// <summary>
            /// Balances this tree.
            /// </summary>
            /// <returns>A balanced tree.</returns>
            private Node Balance() { return this.IsLeftHeavy ? this.BalanceLeft() : this.BalanceRight();
            }
            /// <summary>
            ...
            private static byte ParentHeight(Node left, Node right) { return checked((byte)(1 + Math.Max(left._height, right._height)));
            }
            /// <summary>
            ...
            private static int ParentCount(Node left, Node right) { return 1 + left._count + right._count;
            }
            /// <summary>
            /// Creates a node mutation, either by mutating this node (if not yet frozen) or by creating a clone of this node
            /// with the described changes.
            ...
            {
                return new Node(key, left: EmptyNode, right: EmptyNode);
