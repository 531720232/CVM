...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    ...
    internal partial class LanguageParser : SyntaxParser
    {
        ...
        internal LanguageParser(
            ...
            LexerMode lexerMode = LexerMode.Syntax,CancellationToken cancellationToken=default(CancellationToken))
            : base(lexer, lexerMode, oldTree, changes, allowModeReset: false,
                preLexIfNotIncremental: true,cancellationToken)
        {
        ...
        private ExpressionSyntax ParseCastOrParenExpressionOrLambdaOrTuple(Precedence precedence)
        {
            ...
            try
            {
                ...
                // Doesn't look like a cast, so parse this as a parenthesized expression or tuple.
                {
                    this.Reset(ref resetPoint);
                    var openParen = this.EatToken(SyntaxKind.OpenParenToken);
                    var expression = this.ParseExpressionOrDeclaration(ParseTypeMode.FirstElementOfPossibleTupleLiteral, feature: MessageID.None, permitTupleDesignation: true);
                    ...
                    if (expression.Kind == SyntaxKind.IdentifierName && this.CurrentToken.Kind == SyntaxKind.ColonToken)
                    {
                        ...
                        expression = this.ParseExpressionOrDeclaration(ParseTypeMode.FirstElementOfPossibleTupleLiteral, feature: CSharp.MessageID.None, permitTupleDesignation: true);
        ...
        private TupleExpressionSyntax ParseTupleExpressionTail(SyntaxToken openParen, ArgumentSyntax firstArg)
        {
            ...
            try
            {
                ...
                while (this.CurrentToken.Kind == SyntaxKind.CommaToken)
                {
                    ...
                    var expression = ParseExpressionOrDeclaration(ParseTypeMode.AfterTupleComma, feature: MessageID.None, permitTupleDesignation: true);
                    if (expression.Kind == SyntaxKind.IdentifierName && this.CurrentToken.Kind == SyntaxKind.ColonToken)
                    {
                        var nameColon = _syntaxFactory.NameColon((IdentifierNameSyntax)expression, EatToken());
                        expression = ParseExpressionOrDeclaration(ParseTypeMode.AfterTupleComma, feature: MessageID.None, permitTupleDesignation: true);
