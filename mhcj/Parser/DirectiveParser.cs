            ...
            if (isActive)
            {
                ...
                if (isError)
                {
                    if (errorText.Equals("version", StringComparison.Ordinal))
                    {
                        ...
                        string version = "ve1";// CommonCompiler.GetAssemblyFileVersion(assembly);
                    ...
                    else
                    {
                            ...
                            LanguageVersionFacts.TryParse(errorText.Substring(versionMarker.Length), out var languageVersion))
                        {
                            ...
                            eod = this.AddError(eod, triviaOffset, triviaWidth, error, "version", (languageVersion));
                        }
                    }
                }
            }
