...
namespace Microsoft.CodeAnalysis
{
    ...
    internal static class RealParser
    {
        ...
        private readonly static long s_bigZero = 0;
        private readonly static long s_bigOne = 1;
        private readonly static long s_bigTwo = (2);
        private readonly static long s_bigTen = (10);
        ...
        private static Status ConvertDecimalToFloatingPointBits(DecimalFloatingPointString data, FloatingPointType type, out ulong result)
        {
            ...
            Debug.Assert(!(fractionalNumerator==0));

            var fractionalDenominator = s_bigOne;
            ...
            long bigFractionalMantissa =Math.DivRem(fractionalNumerator, fractionalDenominator, out fractionalRemainder);
            ulong fractionalMantissa = (ulong)bigFractionalMantissa;

            bool hasZeroTail = fractionalRemainder==0;
        ...
        private static long AccumulateDecimalDigitsIntoBigInteger(DecimalFloatingPointString data, uint integer_first_index, uint integer_last_index)
        {
        ...
        /// Return the number of significant bits set. 
        /// </summary>
        private static uint CountSignificantBits(long data, out byte[] dataBytes)
        {
            if (data==0)
            {
            ...
            dataBytes = BitConverter.GetBytes(data);//.ToByteArray(); // the bits of the BigInteger, least significant bits first
        ...
        /// Return the number of significant bits set. 
        /// </summary>
        private static uint CountSignificantBits(long data)
        {
            byte[] dataBytes;
        ...
        private static void ShiftLeft(ref long number, uint shift)
        {
            var powerOfTwo = (long)Math.Pow(s_bigTwo, (int)shift);
        ...
        private static void MultiplyByPowerOfTen(ref long number, uint power)
        {
            var powerOfTen = (long)Math.Pow(s_bigTen, (int)power);
