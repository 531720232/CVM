...
namespace Roslyn.Utilities
{
    internal class TextKeyedCache<T>
    {
        ...
        internal class SharedEntryValue
        {
        ...
        private static readonly HE[] s_sharedTable = new HE[SharedSize];

        // store a reference to shared cache locally
        // accessing a static field of a generic type could be nontrivial
        private readonly HE[] _sharedTableInst = s_sharedTable;
        ...
        internal T FindItem(char[] chars, int start, int len, int hashCode)
        {
            // get direct element reference to avoid extra range checks
             var localSlot =  _localTable[LocalIdxFromHash(hashCode)];
            ...
            {
                var nw = new char[len];
                Array.Copy(chars, start, nw, 0, len);
                if (StringTable.TextEquals(text, nw))
                {
        ...
        private SharedEntryValue FindSharedEntry(char[] chars, int start, int len, int hashCode)
        {
            ...
            for (int i = 1; i < SharedBucketSize + 1; i++)
            {
                ...
                if (e != null)
                {
                    var nw = new char[len];
                    Array.Copy(chars, start, nw, 0, len);
                    if (hash == hashCode && StringTable.TextEquals(e.Text, nw))
                    {
                        break;
                    }
