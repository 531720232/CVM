...
namespace Microsoft.CodeAnalysis
{
    ...
    internal class DiagnosticInfo : IFormattable, IObjectWritable
    {
        ...
        {
            return CVM.AHelper.GetOrAdd(ref s_errorCodeToDescriptorMap, errorCode, code => CreateDescriptor(code, defaultSeverity, messageProvider));
        }
        ...
        protected DiagnosticInfo(ObjectReader reader)
        {
            ...
            if (count == 0)
            {
                _arguments = new object[0];// Array.Empty<object>();
            }
            else if (count > 0)
            {
