...
namespace Microsoft.CodeAnalysis
{
    ...
    internal class DiagnosticBag
    {
        ...
        private static ImmutableArray<TDiagnostic> ToReadOnlyCore<TDiagnostic>(Queue<Diagnostic> oldBag) where TDiagnostic : Diagnostic
        {
            if (oldBag == null)
            {
        ...
        private Queue<Diagnostic> Bag
        {
            get
            {
                ...
                Queue<Diagnostic> newBag = new Queue<Diagnostic>();
                return CVM.AHelper.CompareExchange(ref _lazyBag, newBag, null) ?? newBag;
            }
        }
        ...
        internal sealed class DebuggerProxy
        {
            ...
            public object[] Diagnostics
            {
                get
                {
                    Queue<Diagnostic> lazyBag = _bag._lazyBag;
                    if (lazyBag != null)
                    {
                    ...
                    {
                        return new object[0];//Array.Empty<object>();
                    }
                }
            }
        }
